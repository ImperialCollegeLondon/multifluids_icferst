...
try:
    ...
    debug.deprint(
        "Warning: Failed to import vtk module. This is coming from fluidity_tools.py file."
    )
...
class stat_parser(dict):
    ...
    def __init__(self, filename, subsample=1):
        ...
        if (
            binaryFormat
        ):  # Note: Binary format is true only for .dat files. For .stat files, this statement is not parsed
            for ele in constantEles:
                name = ele.getAttribute("name")
                type = ele.getAttribute("type")
                value = ele.getAttribute("value")
            ...
            for line in statfile:
                ...
                if len(list(entries)) == len(columns) and (lineNo % subsample) == 0:
                    ...
                    raise Exception(
                        ...
                        % (lineNo, len(columns), len(list(entries)))
                    )
                lineNo = lineNo + 1
...
def petsc_memory_stats(log):
    ...
    for line in memory_profile.split("\n"):
        try:
            object, profile = next(re.finditer(r"(\s.*?)([0-9]+.*)", line)).groups()
        except StopIteration:
            continue
        profile = profile.split()
        stats[object.strip()] = [int(x) for x in profile[0:3]] + [
            float(profile[3]),
        ]
