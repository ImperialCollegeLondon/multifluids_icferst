...
def read_fortran_file(filename):
    ...
    module_name = re.search(r"(?<=module )\w+", data, re.I)
    if module_name:
    ...
    modules_used = re.findall(r"(?<=^use )\w+", data, re.MULTILINE + re.I)
    ...
    for module in modules_used:
        ...
        test = ",.+"
        while text[-1] == "&":
            text = re.search("(?<=^use {}){}".format(module, test), data, re.MULTILINE)
            if text:
                text = text.group(0)
            else:
                text = ""
...
def print_misplaced(data, namelist, silent, args):
    ...
    def child_cmp(nm1, nm2):
        ...
        if (
            nm1
            in data.setdefault(nm2, {"indirectly_used_modules": []})[
                "indirectly_used_modules"
            ]
        ):
            return -1
        elif (
            nm2
            in data.setdefault(nm1, {"indirectly_used_modules": []})[
                "indirectly_used_modules"
            ]
        ):
            return 1
...
def add_children(data, name):
    ...
    for child in data[name]["directly_used_modules"]:
        if child in data:
            ...
            data[name]["indirectly_used_modules"] = data[name][
                "indirectly_used_modules"
            ].union(data[child]["indirectly_used_modules"])
        else:
            data[child] = dict(
                filename=None,
...
def make_graphs(tree, namelist):
    ...
    for filename in high_filenames:
        ...
        def add_used(name, out, visited):
            ...
            for module in tree[name]["directly_used_modules"]:
                if module not in visited or (
                    name == namelist[filename][1] and module in namelist[filename][0]
                ):
                    out += "{}->{}\n".format(name, module)
    ...
    dot = subprocess.Popen(
        ["dot", "-Ttiff"], stdin=subprocess.PIPE, stdout=subprocess.PIPE
    )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        action="store_true",
        dest="graph",
        help="generate graph of use statement inheritance.",
        default=False,
    )
    parser.add_option(
        "-p",
