...
class Mesh(events.Evented):
    ...
    def __init__(
        self, dim, nodeCoords=[], volumeElements=[], surfaceElements=[], halos=None
    ):
    ...
    def __str__(self):
        return (
            ...
            + str(self.NodeCount())
            + " nodes, "
            + str(self.VolumeElementCount())
            + " volume elements, "
            + str(self.SurfaceElementCount())
            + " surface elements"
        )
    ...
    def NNList(self):
        ...
        for element in self.GetSurfaceElements() + self.GetVolumeElements():
            ...
            assert (
                type.GetElementFamilyId() == elements.ELEMENT_FAMILY_SIMPLEX
...
def VtuToMesh(vtu, idsName="IDs"):
    ...
    for i in range(vtu.ugrid.GetNumberOfCells()):
        ...
        element = elements.Element(
            nodes=vtktools.FromVtkNodeOrder(
                [nodeIds.GetId(i) for i in range(nodeIds.GetNumberOfIds())], type
            ),
            ids=id,
        )
...
def NodeUniversalNumbers(meshes, level=None):
    ...
    unns = [
        range(bases[i], bases[i] + halo.GetNOwnedNodes())
...
class meshesUnittests(unittest.TestCase):
    def testLowerDimVtuToMesh(self):
        ...
        mesh = VtuToMesh(vtu)
        self.assertEqual(mesh.GetDim(), 1)
        self.assertEqual(mesh.NodeCount(), 2)
        self.assertEqual(mesh.GetNodeCoord(0)[0], 0.0)
        self.assertEqual(mesh.GetNodeCoord(1)[0], 1.0)
    ...
    def testVtuInteroperability(self):
        ...
        self.assertEqual(oldMesh.GetDim(), newMesh.GetDim())
        self.assertEqual(oldMesh.NodeCount(), newMesh.NodeCount())
        self.assertEqual(oldMesh.SurfaceElementCount(), newMesh.SurfaceElementCount())
        self.assertEqual(oldMesh.VolumeElementCount(), newMesh.VolumeElementCount())
