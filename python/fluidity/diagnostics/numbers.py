...
def ThermalBoundaryLayerThickness(g, alpha, deltaT, nu, kappa, H, D=None):
    ...
    return (
        math.pow(
            RayleighNumber(g=g, alpha=alpha, deltaT=deltaT, nu=nu, kappa=kappa, H=H),
            -0.25,
        )
        * D
    )
...
def EkmanBoundaryLayerThickness(omega, nu, H, D=None):
    """
    Calculate the thickness of the Ekman boundary layer in a rotating convection
    problem. As given in P. L. Read, in Rotating Fluids in Geophysical
    Applications, Chapter IV, pages 185-214, CISM Courses and Lectures,
    Springer-Verlag, 1992, p. 195.
