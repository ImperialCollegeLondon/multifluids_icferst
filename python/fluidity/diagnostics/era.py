...
class Era15:
    ...
    def __str__(self):
        return (
            "Latitudes: "
            + str(len(self._latitudes))
            + "\n"
            + "Longitudes: "
            + str(len(self._longitudes) - 1)
            + "\n"
            + "Times: "
            + str(len(self._times))
        )
    ...
    def Value(self, name, index, *indices):
        """
        Index into the supplied field, applying offset, scale and fill and missing
        value detection. Permits indexing into the value directly by supplying
        additional indices.
        """
    ...
    def FieldNames(self):
        """
        Return a list of all fields in the NetCDF data file (including latitude,
        longitude and time)
        """
    ...
    def InterpolatedValue(self, name, latitude, longitude, time):
        ...
        if time < self._times[0] or time > self._times[-1]:
            ...
            debug.deprint(
                "Valid time range = " + str((self._times[0], self._times[-1]))
            )
        ...
        for i, timeIndex in enumerate([before, after]):
            timeVals[i] = calc.BilinearlyInterpolate(
                self.Value(name, timeIndex, upper, left),
                self.Value(name, timeIndex, upper, right),
                self.Value(name, timeIndex, lower, left),
                self.Value(name, timeIndex, lower, right),
                longRatio,
                latRatio,
            )
    ...
    def V10(self, latitude, longitude, time, name="10v"):
        """
        Tri-linearly interpolate the meridional 10m wind at the supplied latitude,
        longitude and time
        """
...
class eraDataUnittests(unittest.TestCase):
    def testEra15(self):
        filename = os.path.join(
            os.path.dirname(__file__), os.path.pardir, "test-data", "ERA15", "output.nc"
        )
        data = Era15(filename)
        data.U10(0.0, 0.0, data.DatetimeToHours(datetime.datetime(1992, 1, 1, 0, 0, 0)))
        data.V10(0.0, 0.0, data.DatetimeToHours(datetime.datetime(1992, 1, 1, 0, 0, 0)))
