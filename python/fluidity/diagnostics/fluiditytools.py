...
def FluidityBinary(
    binaries=["dfluidity-debug", "dfluidity", "fluidity-debug", "fluidity"]
):
    """
    ...
    for possibleBinary in binaries:
        process = subprocess.Popen(
            ["which", possibleBinary], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        process.wait()
        if process.returncode == 0:
            binary = possibleBinary
            debug.dprint(
                "Fluidity binary: " + str(process.stdout.readlines()[0]), newline=False
            )
            break
...
def SplitVtuFilename(filename):
    ...
    ext = (
        utils.FormLine(
            [""] + split[idIndex + 1 : len(split) - 1], delimiter="_", newline=False
        )
        + split[-1]
    )
...
def FindPvtuVtuFilenames(
    project, firstId, lastId=None, pvtuExtension=".pvtu", vtuExtension=".vtu"
):
    """
    Find vtu filenames for a Fluidity simulation, in the supplied range of IDs,
...
def FindFinalVtu(project, extensions=[".vtu", ".pvtu"]):
    ...
    return FindVtuFilenames(
        project, FindMaxVtuId(project, extensions=extensions), extensions=extensions
    )[0]
