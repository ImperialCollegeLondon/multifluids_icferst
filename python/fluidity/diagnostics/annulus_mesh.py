...
def SliceCoordsConstant(minVal, maxVal, divisions):
    ...
    return [
        minVal + (maxVal - minVal) * (float(i) / divisions)
...
def GenerateAnnulusMesh(
    ...
    for i in range(nRCoords - 1):
        debug.dprint(
            "Processing radial element strip "
            + str(i + 1)
            + " of "
            + str(nRCoords - 1),
            2,
        ...
        for j in range(nZCoords - 1):
            debug.dprint(
                "Processing vertical element loop "
                + str(j + 1)
                + " of "
                + str(nZCoords - 1),
                3,
...
def GenerateAnnulusHorizontalSliceMesh(
    rCoords,
    phiCoords,
    innerWallId=1,
    outerWallId=2,
    ...
    for i in range(phiDivisions):
        mesh.AddSurfaceElement(
            elements.Element(
                [rPhiToNode[0][i], rPhiToNode[0][(i + 1) % phiPoints]], innerWallId
            )
        )
    # ... for outer wall
    for i in range(phiDivisions):
        mesh.AddSurfaceElement(
            elements.Element(
                [rPhiToNode[-1][i], rPhiToNode[-1][(i + 1) % phiPoints]], outerWallId
            )
        )
    if not connectEnds:
        # ... for left wall
        for i in range(nRCoords - 1):
            mesh.AddSurfaceElement(
                elements.Element([rPhiToNode[i][0], rPhiToNode[i + 1][0]], leftWallId)
            )
        # ... for right wall
        for i in range(nRCoords - 1):
            mesh.AddSurfaceElement(
                elements.Element(
                    [rPhiToNode[i][-1], rPhiToNode[i + 1][-1]], rightWallId
                )
            )
...
def GenerateAnnulusVerticalIntegralBins(
    nRCoords, nZCoords, nPhiCoords, connectEnds=True
):
    """
...
def GenerateRectangleMesh(
    ...
    if elementFamilyId == elements.ELEMENT_FAMILY_SIMPLEX:
        ...
        for i in range(nXCoords - 1):
            debug.dprint(
                "Processing x element strip " + str(i + 1) + " of " + str(nXCoords - 1),
                2,
            )
            for j in range(nYCoords - 1):
                debug.dprint(
                    "Processing y element strip "
                    + str(i + 1)
                    + " of "
                    + str(nYCoords - 1),
                    3,
                ...
                mesh.AddVolumeElement(
                    elements.Element(
                        [xyToNode[i][j], xyToNode[i + 1][j], xyToNode[i + 1][j + 1]],
                        volumeId,
                    )
                )
                # Triangle 2
                mesh.AddVolumeElement(
                    elements.Element(
                        [xyToNode[i][j], xyToNode[i + 1][j + 1], xyToNode[i][j + 1]],
                        volumeId,
                    )
                )
...
def GenerateCuboidMesh(
    ...
    for i in range(nXCoords - 1):
        debug.dprint(
            "Processing x element strip " + str(i + 1) + " of " + str(nXCoords - 1), 2
        )
        for j in range(nYCoords - 1):
            debug.dprint(
                "Processing y element strip " + str(i + 1) + " of " + str(nYCoords - 1),
                3,
            )
...
class SlopingAnnulusTopAndBottomRemapper(Remapper):
    ...
    def Map(self, coord):
        ...
        newZ = localMinZ + (distanceFromBottom / (self._maxZ - self._minZ)) * (
            localMaxZ - localMinZ
        )
...
class annulus_meshUnittests(unittest.TestCase):
    def testSliceCoordsConstant(self):
        ...
        self.assertAlmostEqual(coords[0], 1.0)
        self.assertAlmostEqual(coords[1], 1.1)
        self.assertAlmostEqual(coords[-2], 1.9)
        self.assertAlmostEqual(coords[-1], 2.0)
    ...
    def testSliceCoordsLinear(self):
        ...
        self.assertAlmostEqual(coords[0], 1.0)
        self.assertAlmostEqual(coords[1], 1.1)
        self.assertAlmostEqual(coords[2], 2.0)
        self.assertAlmostEqual(coords[3], 2.9)
        self.assertAlmostEqual(coords[4], 3.0)
        ...
        for i in range(3):
            self.assertAlmostEqual(
                (coords[i + 2] - coords[i + 1]) / (coords[i + 1] - coords[i]), r
            )
    ...
    def testGenerateAnnulusMesh(self):
        mesh = GenerateAnnulusMesh(
            SliceCoordsConstant(1.0, 2.0, 8),
            SliceCoordsConstant(-1.0, 1.0, 8),
            SliceCoordsConstant(0.0, 2.0 * math.pi * 7.0 / 8.0, 7),
        )
        self.assertEqual(mesh.GetDim(), 3)
        lbound, ubound = mesh.BoundingBox().GetBounds()
        self.assertAlmostEqual(lbound[0], -2.0)
        self.assertAlmostEqual(lbound[1], -2.0)
        self.assertAlmostEqual(lbound[2], -1.0)
        self.assertAlmostEqual(ubound[0], 2.0)
        self.assertAlmostEqual(ubound[1], 2.0)
        self.assertAlmostEqual(ubound[2], 1.0)

        mesh = GenerateAnnulusMesh(
            SliceCoordsConstant(1.0, 2.0, 1),
        ...
        self.assertAlmostEqual(lbound[2], 0.0)
        self.assertAlmostEqual(ubound[0], 2.0)
        self.assertTrue(ubound[1] <= 2.0 + calc.Epsilon())
        self.assertAlmostEqual(ubound[2], 1.0)
        self.assertEqual(mesh.NodeCount(), 32)
        self.assertEqual(mesh.VolumeElementCount(), 48)
        self.assertEqual(mesh.SurfaceElementCount(), 64)
        ...
        self.assertAlmostEqual(ubound[2], 1.0)
        self.assertEqual(mesh.NodeCount(), 32)
        self.assertEqual(mesh.VolumeElementCount(), 42)
        self.assertEqual(mesh.SurfaceElementCount(), 60)
        ...
        self.assertEqual(mesh.VolumeElementCount(), 6)
        self.assertTrue(
            simplices.TetVolume(
                mesh.GetNodeCoords(mesh.GetVolumeElement(0).GetNodes()), signed=True
            )
            > 0.0
        )
        self.assertTrue(
            simplices.TetVolume(
                mesh.GetNodeCoords(mesh.GetVolumeElement(1).GetNodes()), signed=True
            )
            > 0.0
        )
        self.assertTrue(
            simplices.TetVolume(
                mesh.GetNodeCoords(mesh.GetVolumeElement(2).GetNodes()), signed=True
            )
            > 0.0
        )
        self.assertTrue(
            simplices.TetVolume(
                mesh.GetNodeCoords(mesh.GetVolumeElement(3).GetNodes()), signed=True
            )
            > 0.0
        )
        self.assertTrue(
            simplices.TetVolume(
                mesh.GetNodeCoords(mesh.GetVolumeElement(4).GetNodes()), signed=True
            )
            > 0.0
        )
        self.assertTrue(
            simplices.TetVolume(
                mesh.GetNodeCoords(mesh.GetVolumeElement(5).GetNodes()), signed=True
            )
            > 0.0
        )
    ...
    def testGenerateRectangleMesh(self):
        ...
        self.assertEqual(mesh.GetDim(), 2)
        self.assertEqual(mesh.VolumeElementCount(), 128)
        self.assertEqual(mesh.SurfaceElementCount(), 32)
        lbound, ubound = mesh.BoundingBox().GetBounds()
        self.assertAlmostEqual(lbound[0], 1.0)
        self.assertAlmostEqual(lbound[1], -1.0)
        self.assertAlmostEqual(ubound[0], 2.0)
        self.assertAlmostEqual(ubound[1], 1.0)
        self.assertEqual(mesh.NodeCount(), 81)
        self.assertEqual(mesh.VolumeElementCount(), 128)
        self.assertEqual(mesh.SurfaceElementCount(), 32)
    ...
    def testGenerateCuboidMesh(self):
        mesh = GenerateCuboidMesh(
            SliceCoordsConstant(1.0, 2.0, 8),
            SliceCoordsConstant(3.0, 4.0, 8),
            SliceCoordsConstant(5.0, 6.0, 8),
        )
        self.assertEqual(mesh.GetDim(), 3)
        lbound, ubound = mesh.BoundingBox().GetBounds()
        self.assertAlmostEqual(lbound[0], 1.0)
        self.assertAlmostEqual(lbound[1], 3.0)
        self.assertAlmostEqual(lbound[2], 5.0)
        self.assertAlmostEqual(ubound[0], 2.0)
        self.assertAlmostEqual(ubound[1], 4.0)
        self.assertAlmostEqual(ubound[2], 6.0)
        self.assertEqual(mesh.NodeCount(), 9 * 9 * 9)
        self.assertEqual(mesh.VolumeElementCount(), 8 * 8 * 8 * 6)
        self.assertEqual(mesh.SurfaceElementCount(), 8 * 8 * 2 * 6)
    ...
    def testRemapper(self):
        class TestRemapper(Remapper):
            ...
            def Map(self, coord):
                ...
                newCoord[-1] += (
                    self._lift * (self._maxH - newCoord[-1]) / (self._maxH - self._minH)
                )
        ...
        lbound, ubound = mesh.BoundingBox().GetBounds()
        self.assertAlmostEqual(lbound[0], 0.0)
        self.assertAlmostEqual(lbound[1], 1.0)
        ...
        lbound, ubound = mesh.BoundingBox().GetBounds()
        self.assertAlmostEqual(lbound[0], 0.0)
        self.assertAlmostEqual(lbound[1], 1.1)
        self.assertAlmostEqual(ubound[0], 1.0)
        self.assertAlmostEqual(ubound[1], 2.0)
