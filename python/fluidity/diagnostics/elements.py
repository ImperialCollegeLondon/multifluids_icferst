...
(
    ...
    ELEMENT_VERTEX,
    ELEMENT_LINE,
    ELEMENT_QUADRATIC_LINE,
    ELEMENT_TRIANGLE,
    ELEMENT_QUADRATIC_TRIANGLE,
    ELEMENT_QUAD,
    ELEMENT_TETRAHEDRON,
    ELEMENT_QUADRATIC_TETRAHEDRON,
    ELEMENT_HEXAHEDRON,
) = range(11)
...
ELEMENT_FAMILY_UNKNOWN, ELEMENT_FAMILY_SIMPLEX, ELEMENT_FAMILY_CUBIC = range(3)
...
class ElementType(events.Evented):
    ...
    def _SetElementTypeIdFromData(self, dim, nodeCount):
        if (dim, nodeCount) in self._elementTypeIdsMap:
            ...
            debug.deprint(
                "Warning: Unknown element type with "
                + str(nodeCount)
                + " nodes in "
                + str(dim)
                + " dimensions"
...
class elementsUnittests(unittest.TestCase):
    def testElementType(self):
        ...
        self.assertEqual(type.GetElementTypeId(), ELEMENT_QUAD)
        self.assertEqual(type.GetDegree(), 1)
        self.assertEqual(type.GetElementFamilyId(), ELEMENT_FAMILY_CUBIC)
        type.SetDim(3)
        self.assertEqual(type.GetElementTypeId(), ELEMENT_TETRAHEDRON)
        self.assertEqual(type.GetDegree(), 1)
        self.assertEqual(type.GetElementFamilyId(), ELEMENT_FAMILY_SIMPLEX)
        type.SetNodeCount(8)
        self.assertEqual(type.GetElementTypeId(), ELEMENT_HEXAHEDRON)
        self.assertEqual(type.GetDegree(), 1)
        self.assertEqual(type.GetElementFamilyId(), ELEMENT_FAMILY_CUBIC)
