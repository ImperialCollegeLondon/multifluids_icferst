...
class Halo:
    ...
    def __init__(
        self, process, nProcesses, nOwnedNodes=None, sends=None, receives=None
    ):
        assert process >= 0
        assert nProcesses > 0
    ...
    def SetSends(self, sends, process=None):
        if process is None:
            ...
            self._sends = [
                [None for i in range(len(sends[i]))] for i in range(nProcesses)
            ]
    ...
    def SetReceives(self, receives, process=None):
        if process is None:
            ...
            self._receives = [
                [None for i in range(len(receives[i]))] for i in range(nProcesses)
            ]
...
class Halos:
    """
    A set of halos, storing a fixed set of node and element halos (one per level),
    with None for missing halos
    """

    def __init__(self, process, nProcesses, nodeHalos=[], elementHalos=[], nLevels=2):
        self._process = process
        self._nProcesses = nProcesses
    ...
    def HaloCompatible(self, halo):
        return (
            halo.GetProcess() == self.GetProcess()
            and halo.GetNProcesses() == self.GetNProcesses()
        )
        ...
        for i, process in enumerate(range(halo.GetNProcesses())):
            ...
            if level > 0:
                sendText = xmlfile.createTextNode(
                    utils.FormLine(
                        utils.OffsetList(utils.ExpandList(halo.GetSends(process=i)), 1),
                        delimiter=" ",
                        newline=False,
                    )
                )
                ...
                sendText = xmlfile.createTextNode(
                    utils.FormLine(
                        utils.ExpandList(halo.GetSends(process=i)),
                        delimiter=" ",
                        newline=False,
                    )
                )
            ...
            if level > 0:
                receiveText = xmlfile.createTextNode(
                    utils.FormLine(
                        utils.OffsetList(
                            utils.ExpandList(halo.GetReceives(process=i)), 1
                        ),
                        delimiter=" ",
                        newline=False,
                    )
                ...
                receiveText = xmlfile.createTextNode(
                    utils.FormLine(
                        utils.ExpandList(halo.GetReceives(process=i)),
                        delimiter=" ",
                        newline=False,
                    )
                )
...
class mesh_halosUnittests(unittest.TestCase):
    ...
    def testHaloTrailingReceivesOrdered(self):
        ...
        self.assertFalse(
            Halo(
                process=0, nProcesses=1, sends=[[0, 1]], receives=[[2, 3]]
            ).TrailingReceivesOrdered()
        )
        self.assertFalse(
            Halo(
    ...
    def testHalosIO(self):
        halo1 = Halo(
            process=0, nProcesses=1, nOwnedNodes=3, sends=[[0, 1]], receives=[[3, 4]]
        )
        halo2 = Halo(
            process=0,
            nProcesses=1,
        ...
        self.assertEqual(halos.NodeHaloLevels(), [1, 2])
        self.assertEqual(halos.ElementHaloLevels(), [])
        halo1, halo2 = halos.GetNodeHalos()
        self.assertEqual(halo1.GetProcess(), 0)
        self.assertEqual(halo2.GetProcess(), 0)
        self.assertEqual(halo1.GetNProcesses(), 1)
        self.assertEqual(halo2.GetNProcesses(), 1)
        self.assertEqual(halo1.GetSends(process=0), [0, 1])
        self.assertEqual(halo2.GetSends(process=0), [0, 2, 1])
        self.assertEqual(halo1.GetReceives(process=0), [3, 4])
        self.assertEqual(halo2.GetReceives(process=0), [3, 5, 4])
...
class mesh_halosDataUnittests(unittest.TestCase):
    def testReadHalos(self):
        ...
        halos = ReadHalos(filename)
        self.assertEqual(halos.NodeHaloCount(), 2)
        self.assertEqual(halos.GetNodeHalo(1).GetNProcesses(), 4)
        self.assertEqual(halos.GetNodeHalo(2).GetNProcesses(), 4)
