...
def ModelPvtuToVtu(pvtu):
    ...
    result = vtu()
    ghostLevel = pvtu.ugrid.GetCellData().GetArray("vtkGhostType")
    if ghostLevel is None:
        # We have a serial vtu
        debug.deprint("Warning: input file contains no vtkGhostType")
...
def PvtuToVtu(pvtu, model=None, oldNodeIdToNew=[], oldCellIdToNew=[], fieldlist=[]):
    """
...
def VtuFieldGradient(inputVtu, fieldName):
    ...
    # Add the cells
    tempVtu.ugrid.SetCells(
        inputVtu.ugrid.GetCellTypesArray(),
...
def LineVtuProbe(
    inputVtu, fieldName, origin=(0.0, 0.0, 0.0), direction=(1.0, 0.0, 0.0)
):
    """
...
def RingVtuProbe(
    inputVtu, fieldName, radius, origin=(0.0, 0.0, 0.0), normal=(0.0, 0.0, 1.0)
):
    """
...
def RemappedVtu(inputVtu, targetVtu):
    ...
    # Add the cells
    result.ugrid.SetCells(
        targetVtu.ugrid.GetCellTypesArray(),
...
def ZeroVtu(vtu):
    ...
    for fieldName in vtu.GetFieldNames():
        vtu.AddField(
            fieldName,
            ZeroField(
                VtuFieldComponents(vtu, fieldName), VtuFieldTuples(vtu, fieldName)
            ),
        )
...
def TranslateVtu(vtu, translation):
    ...
    for location in locations:
        newLocations.InsertNextPoint(
            [comp + translation[i] for i, comp in enumerate(location)]
        )
...
def VtuBoundingBox(vtu):
    ...
    lbound, ubound = [vtuBounds[2 * i] for i in range(int(len(vtuBounds) / 2))], [
        vtuBounds[2 * i + 1] for i in range(int(len(vtuBounds) / 2))
    ]
...
def VtuStripFloatingNodes(vtu):
    ...
    vtu.ugrid.SetCells(
        vtu.ugrid.GetCellTypesArray(), vtu.ugrid.GetCellLocationsArray(), cells
    )
