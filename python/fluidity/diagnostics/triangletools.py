    ...
    if hasFace and dim > 2:
        ...
        for line in faceHandle.readlines():
            ...
            for node in lineSplit[1 : len(lineSplit) - nFaceIds]:
                element.AddNode(int(node) - 1)
            element.SetIds(
                [int(faceId) for faceId in lineSplit[len(lineSplit) - nFaceIds :]]
            )
            mesh.AddSurfaceElement(element)
    ...
    if hasEle:
        ...
        for line in eleHandle.readlines():
            ...
            for node in lineSplit[1 : len(lineSplit) - nEleIds]:
                element.AddNode(int(node) - 1)
            element.SetIds(
                [int(eleId) for eleId in lineSplit[len(lineSplit) - nEleIds :]]
            )
...
def WriteTriangle(mesh, baseName):
    ...
    def FileFooter():
        return (
            "# Created by triangletools.WriteTriangle\n"
            + "# Command: "
            + " ".join(sys.argv)
            + "\n"
            + "# "
    ...
    for i in range(mesh.VolumeElementCount()):
        ...
        eleHandle.write(
            utils.FormLine(
                [
                    i + 1,
                    utils.OffsetList(mesh.GetVolumeElement(i).GetNodes(), 1),
