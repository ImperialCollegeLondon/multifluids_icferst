...
class Field:
    ...
    def remap_ele(self, ele_number, mesh):
        ...
        assert not (
            self.mesh.shape.type == "bubble" and mesh.shape.type == "lagrangian"
        )
...
class Mesh:
    ...
    def __init__(
        self, ndglno, elements, nodes, continuity, name, option_path, region_ids
    ):
        self.ndglno = ndglno
...
class Element:
    ...
    def __init__(
        ...
        self.spoly = [
            [Polynomial([], 0) for inner in range(size_spoly_y)]
            for outer in range(size_spoly_x)
        ]
        self.dspoly = [
            [Polynomial([], 0) for inner in range(size_dspoly_y)]
...
class Quadrature:
    ...
    def __init__(self, w, locations, dim, degree, loc, ngi):
        self.dimension = (
            dim  # Dimension of the elements for which quadrature is required.
        )
...
class Transform:
    ...
    def __init__(self, ele_num, field):
        ...
        self.invJ = [
            numpy.zeros((self.element.dimension, field.dimension))
            for gi in range(self.element.ngi)
        ]
    ...
    def set_J(self, J, gi):
        ...
        self.det[gi] = abs(
            functools.reduce(lambda x, y: x * y, [s for s in S if s > 0], 1)
        )
        ...
        for i in range(shape.loc):
            for j in range(dshape_loc):
                shape_dshape[i, j, :] = (
                    ...
                    * numpy.matrix(self.spread(shape.n[i], dshape_dim) * dshape.dn[j])
                ).reshape((dshape_dim,))
        return shape_dshape
