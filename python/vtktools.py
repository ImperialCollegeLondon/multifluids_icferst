...
class vtu:
    ...
    def __init__(self, filename=None):
        ...
        if filename is None:
            ...
            if self.ugrid.GetNumberOfPoints() + self.ugrid.GetNumberOfCells() == 0:
                raise Exception(
                    "ERROR: No points or cells found after loading vtu " + filename
                )
    ...
    def GetCellVolume(self, id):
        ...
        if isinstance(cell, vtk.vtkTriangle):
            ...
            return abs(
                cell.ComputeVolume(
                    pts.GetPoint(0), pts.GetPoint(1), pts.GetPoint(2), pts.GetPoint(3)
                )
            )
        elif cell.GetNumberOfPoints() == 3:
            return abs(
                cell.ComputeVolume(pts.GetPoint(0), pts.GetPoint(1), pts.GetPoint(2))
            )
        else:
...
def VtuMatchLocationsArbitrary(vtu1, vtu2, tolerance=1.0e-6):
    ...
    for j in range(locations1.shape[1]):
        ...
        if tolerance < epsilon:
            ...
            raise Exception(
                "ERROR: specified tolerance is smaller than machine precision of given locations"
            )
    ...
    return numpy.allclose(
        locations1[sort_index1], locations2[sort_index2], atol=tolerance
    )
