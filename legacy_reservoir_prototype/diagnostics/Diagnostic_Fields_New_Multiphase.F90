...
module diagnostic_fields_new_multiphase
...
contains
  ...
  subroutine calculate_diagnostic_variables(states, exclude_nonrecalculated)
    ...
    ewrite(1, *) "In calculate_diagnostic_variables MP"
...
! k-epsilon momentum diagnostics (reynolds stress tensor)
    ...
    if(have_option(trim(states(1)%option_path)//&
         "/subgridscale_parameterisations/k-epsilon")) then
       call keps_momentum_diagnostics(states(1))
       print *, 'k-epsilon momentum diagnostics'
    end if
  ...
  subroutine calculate_diagnostic_variable_scalar_multiple_indexed(states, state_index, s_field, algorithm, stat)
    ...
    select case(trim(lalgorithm))
      ...
      case("surface_horizontal_divergence")
        call calculate_surface_horizontal_divergence(state, s_field)
      ...
      case("strain_rate_second_invariant")
        call calculate_strain_rate_second_invariant(state, s_field)
  ...
  subroutine calculate_diagnostic_variable_vector_multiple_indexed(states, state_index, v_field, algorithm, stat)
    ...
    select case(trim(lalgorithm))
      ...
      case("darcy_velocity")
        call calculate_darcy_velocity(states, state_index, v_field)
...
end module diagnostic_fields_new_multiphase
