...
contains
    ...
    subroutine CheckElementAngles(X_ALL, totele, x_ndgln, X_nloc, MaxAngle, Quality_list, number_bad)
    ...
    if (size(X_ALL,1)==2) then!2D triangles
        ...
        do ELE = 1, totele
                ...
                if (Bad_found) then
                    ...
                    Bad_found = Check_element(X_ALL, x_ndgln, (ele-1)*X_nloc, 1, 2, 4, MxAngl, Quality_list(ele))
                    if (Bad_found) then
                        k = k + 1
                        Quality_list(ele)%bad_ele = ele
                    else
                        Bad_found = Check_element(X_ALL, x_ndgln, (ele-1)*X_nloc, 1, 4, 3, MxAngl, Quality_list(ele))
                        if (Bad_found) then
                            k = k + 1
                            Quality_list(ele)%bad_ele = ele
                        else
                            Bad_found = Check_element(X_ALL, x_ndgln, (ele-1)*X_nloc, 4, 2, 3, MxAngl, Quality_list(ele))
                            if (Bad_found) then
                                k = k + 1
                                Quality_list(ele)%bad_ele = ele
                            end if
    ...
    if (present(number_bad)) then
        number_bad = k ! pass for the diagnostic table
    else
        if (float(k)/totele > 0.) then ! if bad elements found, let the user know
            ewrite(0,'(A,F6.2,A,G7.2,A,I3,A)') 'WARNING:', (float(k)/totele)*100., '% of the elements, (',  k  ,') are bad elements, with an angle larger than ', int(MaxAngle), ' degrees'
        end if
    end if
contains
