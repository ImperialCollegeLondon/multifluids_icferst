...
module multiphase_fractures_3D
  ...
  interface
     subroutine y3d_allocate_femdem( string, &
          ...
          &     nodes_v, elements_v, edges_v, iflag )
...
!        return 0
  ...
  subroutine initialise_femdem
    ...
    integer, dimension( : ), allocatable :: ele1_r, ele2_r, ele3_r,ele4_r, &
         &                                  ele1_v, ele2_v, ele3_v, ele4_v, &
         &                                  face1_r, face2_r,face3_r, face1_v, face2_v, face3_v,iflag
...
!    iflag(3)=0
    ...
    call y3d_allocate_femdem( trim( femdem_mesh_name ) // char( 0 ), &
         nodes_r, elements_r, edges_r, nodes_v, elements_v, edges_v,iflag )
...
!! -ao problem here for overlapping ring mesh
    call add_faces( positions_r%mesh, &
         sndgln = sndglno_r, &
         boundary_ids = boundary_ids_r, allow_duplicate_internal_facets=.true.)
...
!    print *,  'point11- '
    ...
    call set_all( permeability_r, 1, 3, p3 )
    call set_all( permeability_r, 2, 1, p4 )
    call set_all( permeability_r, 2, 2, p5 )
    call set_all( permeability_r, 2, 3, p6 )
    call set_all( permeability_r, 3, 1, p7 )
    call set_all( permeability_r, 3, 2, p8 )
    call set_all( permeability_r, 3, 3, p9 )
    ...
    deallocate( ele1_r, ele2_r, ele3_r, ele4_r, face1_r, face2_r,face3_r, &
        ...
        &      x_r, y_r, z_r, x_v, y_v, z_v, p1, p2, p3, p4, p5, p6, p7, p8, p9, iflag )
...
print *,  'leaving calculate_absorption'
  ...
  subroutine calculate_phi_and_perm( packed_state, state )
    ...
    type( tensor_field ), pointer :: permeability, perm_state, FracMap
        type( scalar_field ), pointer :: vf , poro_state, perm_val, perm2_val
...
!     call insert( alg_ext, field_ext_p31, "Permeability31" )
    ...
    print *, minval(field_ext_p11)
	print *, maxval(field_ext_p11)

	print *, minval(field_ext_p22)
	print *, maxval(field_ext_p22)

	print *, minval(field_ext_p33)
	print *, maxval(field_ext_p33)
    ...
    print *, minval(field_fl_p33)
    print *, maxval(field_fl_p33)
    ...
    allocate( perm(ndim, ndim, totele) ) ; perm= permeability % val
    ...
    do ele=1, totele
       ...
       if ((rvf % val (ele) > 0.0)) then 
             ...
             if (field_fl_p22%val(ele)>bg_perm) then
                perm( 2, 2, ele ) = field_fl_p22 % val (ele)
            end if
             if (field_fl_p33%val(ele)>bg_perm) then
                perm( 3, 3, ele ) = field_fl_p33 % val (ele)
            end if
       ! non-normalised and conservative permeability interpolation

            perm( 1, 2, ele ) = 0. !field_fl_p12 % val (ele)
            perm( 2, 1, ele ) = 0. ! field_fl_p21 % val (ele)

            perm( 1, 3, ele ) =0.  ! field_fl_p13 % val (ele)
            perm( 2, 3, ele ) = 0. ! field_fl_p23 % val (ele)

            perm( 3, 2, ele ) = 0. ! field_fl_p32 % val (ele)
            perm( 3, 1, ele ) = 0. ! field_fl_p31 % val (ele)
         end if
    end do
...
!!-PY this is a big problem 
          ...
          if ( constant_mu ) then
                 ...
                 field_fl_mu % val(fl_ele_nodes( 2 ) ) = viscosity % val( 1, 1, 1 )
                 field_fl_mu % val(fl_ele_nodes( 3 ) ) = viscosity % val( 1, 1, 1 )
                 field_fl_mu % val(fl_ele_nodes( 4 ) ) = viscosity % val( 1, 1, 1 )
          else
                 field_fl_mu % val(fl_ele_nodes( 1 ) ) = viscosity % val( 1, 1, cv_ndgln( ( ele - 1 ) * cv_nloc + 1 ) )
                 field_fl_mu % val(fl_ele_nodes( 2 ) ) = viscosity % val( 1, 1, cv_ndgln( ( ele - 1 ) * cv_nloc + 3 ) )
                 field_fl_mu % val(fl_ele_nodes( 3 ) ) = viscosity % val( 1, 1, cv_ndgln( ( ele - 1 ) * cv_nloc + 6 ) )
                 field_fl_mu % val(fl_ele_nodes( 4 ) ) = viscosity % val( 1, 1, cv_ndgln( ( ele - 1 ) * cv_nloc + 10 ) )
          end if
       end do
    else
     ...
     if (.false.) then
            open  (unit=101,file="fluid_viscosity.txt",action="write",status="replace")
            do i=1, size(field_fl_mu%val)
                 write(101,*), viscosity%val(1,1,i)
            end do
            close (101)
     end if
...
print *,  'interpolate_fields_out_r, 08-0'
 ...
 print *,  'interpolate_fields_out_v, 03'
  ...
  subroutine interpolate_fields_in_v( packed_state, du_s, u_s )
    ...
    !Local variables
    type( mesh_type ), pointer :: p0_fl_mesh, fl_mesh, u_mesh, p_mesh
    type( scalar_field ), pointer :: solid, solid_t, old_solid, f
    type( scalar_field ) :: field_fl_du, field_fl_dv,field_fl_dw, &
         ...
         &                  field_ext_us, field_ext_vs,field_ext_ws, f2, f3, &
...
!!-PY change it to make SolidConcentration correct
!!    solid => extract_scalar_field( packed_state, "SolidConcentration" )
    !!solid => extract_scalar_field( alg_fl, "SolidConcentration" )
...
!!    solid_t => extract_scalar_field( packed_state, "SolidConcentration" )
!!    call set( solid_t, solid )
...
!        !-ao--------------------------------debugging--------------------------------------------------------------------------
            open  (unit=100,file="projection_pressure.txt",action="write",status="replace")
            do ele=1, size(p_v)
                 write(100,*) "Pressure", ele, p_v(ele)
            end do
            close (100)
!        !-ao--------------------------------debugging--------------------------------------------------------------------------
