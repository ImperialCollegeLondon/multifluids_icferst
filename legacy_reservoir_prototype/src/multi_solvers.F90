...
#include "petsc_legacy.h"
    ...
    public :: multi_solver, BoundedSolutionCorrections, FPI_backtracking, Set_Saturation_to_sum_one,&
         Ensure_initial_Saturation_to_sum_one, FPI_backtracking_for_temperature, auto_backtracking
...
contains
    ...
    subroutine BoundedSolutionCorrections( state, packed_state, &
        ...
        integer :: ndim1, ndim2,  i, j, knod, inod, jnod, count, ii, jj, loc_its, loc_its2, its, gl_its
        ...
        !Establish bounds
        if (present_and_true(for_sat)) then
            if (present(IDs2CV_ndgln)) then!if this is present then the bounds are applied using immobile fractions
            ...
            else!Otherwise, the bounds are between o and 1, which is not the usual case
...
!        integer, parameter :: History_order = 4!<= Cubic
    ...
    subroutine FPI_backtracking_for_temperature(state, packed_state, temp_bak, backtrack_temp, backtrack_par_from_schema, &
        Previous_convergence, satisfactory_convergence, new_backtrack_par, its, nonlinear_iteration, useful_temps, res, &
        res_ratio, first_res, npres, totally_min_max)
        !In this subroutine we applied some corrections and backtrack_par on the temperature obtained from the temperature equation
        !this idea is based on the paper SPE-173267-MS.
        !The method ensures convergence "independent" of the time step.
        implicit none
        !Global variables
        type( state_type ), dimension( : ), intent( in ) :: state
        type( state_type ), intent(inout) :: packed_state
        real, dimension(:, :), intent(in) :: temp_bak, backtrack_temp
        real, intent(in) :: backtrack_par_from_schema, res, res_ratio, first_res
        logical, intent(inout) :: satisfactory_convergence
        real, intent(inout) :: new_backtrack_par, Previous_convergence
        integer, intent(in) :: its, nonlinear_iteration, npres
        integer, intent(inout) :: useful_temps
        real, dimension(2), intent(inout) :: totally_min_max
        !Local parameters
        real, parameter :: Conv_to_achiv = 10.0
        real, save :: anders_exp!This parameter change the importance of backtrack_temp in Anderson's acceleration (mainly for high alphas)
        !Local variables        !100 => backtrack_temp is not used; 0.3 => equally important; 0.4 => recommended; 0 => more important than temp_bak
...
!        integer, parameter :: History_order = 4!<= Cubic
        ...
        real, save :: convergence_tol
        real, save :: Infinite_norm_tol
        type (tensor_field), pointer :: temp_field
        !Boundary conditions for min/max principle
        ...
        if (backtrack_par_from_schema < 0.0) then
            ...
            if (backtrack_pars(1) < 0) then!retrieve it just once
                ...
                call get_option( '/timestepping/nonlinear_iterations/Fixed_Point_Iteration/Infinite_norm_tol',&
                    Infinite_norm_tol, default = 0.03 )
                backtrack_pars(1) = max(min(abs(backtrack_par_from_schema), 1.0), min_backtrack)
                !Retrieve the shape of the function to use to weight the importance of previous temperatures
                call get_option( '/timestepping/nonlinear_iterations/Fixed_Point_Iteration/Acceleration_exp',&
                    anders_exp, default = 0.4 )!0.4 the best option, based on experience
                !Should not be negative
                anders_exp = max(anders_exp, 0.)
            ...
            if (new_time_step .and.is_porous_media) then
                ...
                !If a backtrack_par parameter turns out not to be useful, then undo that iteration
                if (its > 2 .and. Convergences(2) > 0 .and. allow_undo .and. Convergences(1)>5.) then
                    temperature = backtrack_temp
                    !We do not allow two consecutive undos
                    allow_undo = .false.
                    !restart the counter of useful temperatures, this is to not use Anderson acceleration
                    !with values that were rejected
                    useful_temps = 1
                    !Undo de accumulation of backtrack_par parameters
                    new_backtrack_par = - backtrack_pars(1)
                ...
                !Select different backtrack_par parameter for the first temperature iteration (SFPI)
                select case (its)
                    case (1)
                        !First, use the one introduced by the user, this is necessary since the algorithm tends to small
                        !values of alphas
            ...
            backtrack_pars(1) = backtrack_par_from_schema
            !Just one local temperature iteration
            satisfactory_convergence = .true.
        end if
        ...
        if (useful_temps < 2 .or. satisfactory_convergence ) then
            ...
            !Based on making backtrack_temp small when backtrack_pars(1) is high and backtrack_temp small when backtrack_pars(1) is small
            !The highest value of backtrack_temp is displaced to low values of alpha
            aux = 1.0 - backtrack_pars(1)
            temperature = backtrack_pars(1) * temperature + aux * ( (1.-(aux**anders_exp *backtrack_pars(1)) ) * temp_bak + &
                aux**anders_exp *backtrack_pars(1) * backtrack_temp)!<=The best option so far

        end if
        !Update halos with the new values
        if (IsParallel()) call halo_update(temp_field)
        !Inform of the new backtrack_par parameter used
        new_backtrack_par = backtrack_pars(1)
    ...
    end subroutine FPI_backtracking_for_temperature
