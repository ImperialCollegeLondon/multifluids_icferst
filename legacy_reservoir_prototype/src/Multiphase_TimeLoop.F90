        ...
        subroutine adapt_mesh_mp()
            ...
            Conditional_ReallocatingFields: if( do_reallocate_fields ) then
                ...
                call Defining_MaxLengths_for_Sparsity_Matrices( ndim, nphase, totele, u_nloc, cv_nloc, ph_nloc, cv_nonods, &
                    ...
                    mx_ncolacv, mx_ncolm, mx_ncolph )
                nlenmcy = u_nonods * nphase * ndim + cv_nonods
                allocate( Mspars%ACV%fin( cv_nonods * nphase + 1 ), Mspars%ACV%col( mx_ncolacv ), Mspars%ACV%mid( cv_nonods * nphase ), &
                    Mspars%MCY%fin( nlenmcy + 1 ), Mspars%MCY%col( mx_ncolmcy ), Mspars%MCY%mid( nlenmcy ), &
                    Mspars%DGM_PHA%fin( u_nonods * nphase * ndim + 1 ), Mspars%DGM_PHA%col( mx_ncoldgm_pha ), &
                    Mspars%DGM_PHA%mid( u_nonods * nphase * ndim ), &
                    Mspars%CT%fin( cv_nonods + 1 ), Mspars%CT%col( mx_nct ), &
                    Mspars%C%fin( u_nonods + 1 ), Mspars%C%col( mx_nc ), &
                    Mspars%CMC%fin( cv_nonods + 1 ), Mspars%CMC%col( mx_ncolcmc ), Mspars%CMC%mid( cv_nonods ), &
                    Mspars%M%fin( cv_nonods + 1 ), Mspars%M%col( mx_ncolm ), Mspars%M%mid( cv_nonods ), &
                    Mspars%ph%fin( ph_nonods + 1 ), Mspars%ph%col( mx_ncolph ) )
                Mspars%ACV%fin = 0 ; Mspars%ACV%col = 0 ; Mspars%ACV%mid = 0 ; Mspars%MCY%fin = 0 ; Mspars%MCY%col = 0 ; Mspars%MCY%mid = 0 ; &
                    Mspars%DGM_PHA%fin = 0 ; Mspars%DGM_PHA%col = 0 ; Mspars%DGM_PHA%mid = 0 ; Mspars%CT%fin = 0 ; &
                    Mspars%CT%col = 0 ; Mspars%C%fin = 0 ; Mspars%C%col = 0 ; Mspars%CMC%fin = 0 ; Mspars%CMC%col = 0 ; Mspars%CMC%mid = 0 ; Mspars%M%fin = 0 ; &
