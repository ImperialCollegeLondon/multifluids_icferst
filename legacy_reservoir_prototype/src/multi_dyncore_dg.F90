...
contains
  ...
  END SUBROUTINE INTENERGE_ASSEM_SOLVE
    ...
    SUBROUTINE FORCE_BAL_CTY_ASSEM_SOLVE( state, packed_state, Mdims, CV_GIdims, CV_funs, FE_funs, storage_state, &
         velocity, pressure, &
      ...
      ELSE ! solve using a projection method
         ...
         if ( high_order_Ph ) then
            if ( .not. ( after_adapt .and. cty_proj_after_adapt ) ) then
               call high_order_pressure_solve( u_rhs, state, packed_state, storage_state, StorageIndexes, Mdims, &
                    cv_ele_type, Mdims%nphase, U_absorbin )
            end if
         end if
...
! NEED TO CHANGE RETRIEVE_SOLID_CTY TO MAKE AN OPTION
        ...
        CALL ASSEMB_FORCE_CTY( state, packed_state, storage_state, Mdims, &
             velocity, pressure, &
...
!    END SUBROUTINE PUT_MOM_C_IN_GLOB_MAT
    ...
    SUBROUTINE ASSEMB_FORCE_CTY( state, packed_state,storage_state, Mdims, &
         velocity, pressure, &
...
!!$        INTEGER :: CV_NGI, CV_NGI_SHORT, SCVNGI, SBCVNGI, NFACE
      ...
      !The different quadrature set
      QUAD_OVER_WHOLE_ELE = .not. GET_C_IN_CV_IN_DYNCORE
...
!!$            cv_ngi, cv_ngi_short, scvngi, sbcvngi, nface, QUAD_OVER_WHOLE_ELE)
      ...
      ALLOCATE( UD( NDIM_VEL, NPHASE, GIDIMS%CV_NGI ))
      ALLOCATE( UDOLD( NDIM_VEL, NPHASE, GIDIMS%CV_NGI ))

      ALLOCATE( UD_ND( NDIM, NPHASE, GIDIMS%CV_NGI ))
      ALLOCATE( UDOLD_ND( NDIM, NPHASE, GIDIMS%CV_NGI ))

      ALLOCATE( DENGI( NPHASE, GIDIMS%CV_NGI ))
      ALLOCATE( DENGIOLD( NPHASE, GIDIMS%CV_NGI ))
      ALLOCATE( GRAD_SOU_GI( NPHASE, GIDIMS%CV_NGI ))
      ...
      ALLOCATE( SIGMAGI( NDIM_VEL * NPHASE, NDIM_VEL * NPHASE, GIDIMS%CV_NGI ))
      ALLOCATE( SIGMAGI_STAB( NDIM_VEL * NPHASE, NDIM_VEL * NPHASE, GIDIMS%CV_NGI ))
      ALLOCATE( MAT_M( MAT_NLOC, GIDIMS%CV_NGI ))
      ALLOCATE( SNORMXN( GIDIMS%SBCVNGI ))
      ALLOCATE( SNORMYN( GIDIMS%SBCVNGI ))
      ALLOCATE( SNORMZN( GIDIMS%SBCVNGI ))

      ALLOCATE( XL_ALL(NDIM,CV_NLOC), XL2_ALL(NDIM,CV_NLOC), XSL_ALL(NDIM,CV_SNLOC) )
      ALLOCATE( NORMX_ALL(NDIM), SNORMXN_ALL(NDIM,GIDIMS%SBCVNGI) )
      ...
      ALLOCATE(NN_SIGMAGI_ELE( NDIM_VEL * NPHASE, NDIM_VEL * NPHASE, U_NLOC, U_NLOC )); NN_SIGMAGI_ELE = 0.
      ALLOCATE(NN_SIGMAGI_STAB_ELE( NDIM_VEL * NPHASE, NDIM_VEL * NPHASE, U_NLOC, U_NLOC )); NN_SIGMAGI_STAB_ELE = 0.
      ALLOCATE(NN_MASS_ELE( NDIM_VEL * NPHASE, NDIM_VEL * NPHASE, U_NLOC, U_NLOC )); NN_MASS_ELE = 0.
      ALLOCATE(NN_MASSOLD_ELE( NDIM_VEL * NPHASE, NDIM_VEL * NPHASE, U_NLOC, U_NLOC )); NN_MASSOLD_ELE = 0.
      ALLOCATE( STRESS_IJ_ELE( NDIM, NDIM, NPHASE, U_NLOC,U_NLOC )); STRESS_IJ_ELE = 0.
      ALLOCATE( VLK_ELE( NPHASE, U_NLOC, U_NLOC )); VLK_ELE = 0.

      ALLOCATE( CENT_RELAX(NDIM,NPHASE,GIDIMS%SBCVNGI) ,CENT_RELAX_OLD(NDIM,NPHASE,GIDIMS%SBCVNGI) )
      ...
      IF(GOT_VIRTUAL_MASS) THEN
         ...
         ALLOCATE( VIRTUAL_MASS_GI( NPHASE, NPHASE, GIDIMS%CV_NGI_SHORT), VIRTUAL_MASS_OLD_GI( NPHASE, NPHASE, GIDIMS%CV_NGI_SHORT))
         ALLOCATE( VLN_CVM( NPHASE,NPHASE ), VLN_OLD_CVM( NPHASE,NPHASE ) )
         ALLOCATE( CVM_SNDOTQ_IN(NDIM,NPHASE,NPHASE,GIDIMS%SBCVNGI), CVM_SNDOTQ_OUT(NDIM,NPHASE,NPHASE,GIDIMS%SBCVNGI) )
         ALLOCATE( CVM_SNDOTQOLD_IN(NDIM,NPHASE,NPHASE,GIDIMS%SBCVNGI), CVM_SNDOTQOLD_OUT(NDIM,NPHASE,NPHASE,GIDIMS%SBCVNGI) )
         ALLOCATE( CVM_NN_SNDOTQ_IN(NPHASE), CVM_NN_SNDOTQ_OUT(NPHASE), CVM_NN_SNDOTQOLD_IN(NPHASE), CVM_NN_SNDOTQOLD_OUT(NPHASE) )
         ALLOCATE( SVIRTUAL_MASS_GI(NPHASE,NPHASE,GIDIMS%SBCVNGI) , SVIRTUAL_MASS_OLD_GI(NPHASE,NPHASE,GIDIMS%SBCVNGI) )
         ALLOCATE( SVIRTUAL_MASS_GI_KEEP(NPHASE,NPHASE,GIDIMS%SBCVNGI), SVIRTUAL_MASS_GI2_KEEP(NPHASE,NPHASE,GIDIMS%SBCVNGI) )
         ALLOCATE( SVIRTUAL_MASS_OLD_GI_KEEP(NPHASE,NPHASE,GIDIMS%SBCVNGI), SVIRTUAL_MASS_OLD_GI2_KEEP(NPHASE,NPHASE,GIDIMS%SBCVNGI) )
      ...
      ALLOCATE( NXUDN( GIDIMS%SCVNGI ))

      ALLOCATE( SDETWE( GIDIMS%SBCVNGI ))
      ...
      ALLOCATE( TEN_XX( NDIM, NDIM, NPHASE, GIDIMS%CV_NGI ))
      ALLOCATE( TEN_VOL( NPHASE, GIDIMS%CV_NGI ))

      ALLOCATE( VLN( NPHASE ))
      ALLOCATE( VLN_OLD( NPHASE ))
      ALLOCATE( VLK( NPHASE ))

      ALLOCATE( SUD_ALL(NDIM,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SUDOLD_ALL(NDIM,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SUD2_ALL(NDIM,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SUDOLD2_ALL(NDIM,NPHASE,GIDIMS%SBCVNGI) )

      ALLOCATE( SNDOTQ(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SNDOTQOLD(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SNDOTQ_ROE(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SNDOTQOLD_ROE(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SINCOME(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SINCOMEOLD(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SDEN(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SDENOLD(NPHASE,GIDIMS%SBCVNGI) )

      ALLOCATE( SDEN_KEEP(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SDENOLD_KEEP(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SDEN2_KEEP(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SDENOLD2_KEEP(NPHASE,GIDIMS%SBCVNGI) )

      ALLOCATE( SUD_ALL_KEEP(NDIM,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SUDOLD_ALL_KEEP(NDIM,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SUD2_ALL_KEEP(NDIM,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SUDOLD2_ALL_KEEP(NDIM,NPHASE,GIDIMS%SBCVNGI) )

      ALLOCATE( SNDOTQ_KEEP(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SNDOTQ2_KEEP(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SNDOTQOLD_KEEP(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SNDOTQOLD2_KEEP(NPHASE,GIDIMS%SBCVNGI) )

      ALLOCATE( N_DOT_DU(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( N_DOT_DU2(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( N_DOT_DUOLD(NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( N_DOT_DUOLD2(NPHASE,GIDIMS%SBCVNGI) )

      ALLOCATE( vel_dot(GIDIMS%SBCVNGI), vel_dot2(GIDIMS%SBCVNGI), velold_dot(GIDIMS%SBCVNGI), velold_dot2(GIDIMS%SBCVNGI), grad_fact(GIDIMS%SBCVNGI) )

      ALLOCATE( DIFF_COEF_DIVDX(NDIM_VEL,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( DIFF_COEFOLD_DIVDX(NDIM_VEL,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( FTHETA(NDIM_VEL,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SNDOTQ_IN(NDIM_VEL,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SNDOTQ_OUT(NDIM_VEL,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SNDOTQOLD_IN(NDIM_VEL,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( SNDOTQOLD_OUT(NDIM_VEL,NPHASE,GIDIMS%SBCVNGI) )
      ...
      ALLOCATE( DIFFGI_U(NDIM_VEL,NPHASE,GIDIMS%CV_NGI) )

      ALLOCATE( U_DT(NDIM_VEL, NPHASE,GIDIMS%CV_NGI) )


      ALLOCATE( U_DX_ALL( NDIM, NDIM_VEL, NPHASE, GIDIMS%CV_NGI ) )
      ALLOCATE( UOLD_DX_ALL( NDIM, NDIM_VEL, NPHASE, GIDIMS%CV_NGI ) )

      ALLOCATE( UOLD_DX(GIDIMS%CV_NGI,NPHASE), UOLD_DY(GIDIMS%CV_NGI,NPHASE), UOLD_DZ(GIDIMS%CV_NGI,NPHASE) )
      ALLOCATE( VOLD_DX(GIDIMS%CV_NGI,NPHASE), VOLD_DY(GIDIMS%CV_NGI,NPHASE), VOLD_DZ(GIDIMS%CV_NGI,NPHASE) )
      ALLOCATE( WOLD_DX(GIDIMS%CV_NGI,NPHASE), WOLD_DY(GIDIMS%CV_NGI,NPHASE), WOLD_DZ(GIDIMS%CV_NGI,NPHASE) )

      ALLOCATE( SOUGI_X(NDIM_VEL,NPHASE,GIDIMS%CV_NGI) )


      ALLOCATE( RESID_U(NDIM_VEL,NPHASE,GIDIMS%CV_NGI) )
      ALLOCATE( P_DX(NDIM, GIDIMS%CV_NGI)  )

      ALLOCATE( U_GRAD_NORM2(NDIM_VEL,NPHASE,GIDIMS%CV_NGI), U_GRAD_NORM(NDIM_VEL,NPHASE,GIDIMS%CV_NGI) )


      ALLOCATE( A_DOT_U(NDIM_VEL,NPHASE,GIDIMS%CV_NGI) )
      ALLOCATE( STAR_U_COEF(NDIM_VEL,NPHASE,GIDIMS%CV_NGI) )
      ALLOCATE( P_STAR_U(NDIM_VEL,NPHASE,GIDIMS%CV_NGI) )
      ALLOCATE( DIF_STAB_U(NDIM_VEL,NPHASE, GIDIMS%CV_NGI) )
      ...
      ALLOCATE( UFENX_JLOC_U(NDIM,NDIM,GIDIMS%CV_NGI,U_NLOC) )
      ...
      ALLOCATE( U_NODI_SGI_IPHASE_ALL(NDIM_VEL,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( U_NODJ_SGI_IPHASE_ALL(NDIM_VEL,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( UOLD_NODI_SGI_IPHASE_ALL(NDIM_VEL,NPHASE,GIDIMS%SBCVNGI) )
      ALLOCATE( UOLD_NODJ_SGI_IPHASE_ALL(NDIM_VEL,NPHASE,GIDIMS%SBCVNGI) )
      ...
      IF(IDIVID_BY_VOL_FRAC+IGOT_VOL_X_PRESSURE.GE.1) THEN
         ALLOCATE( VOL_FRA_GI(NPHASE, GIDIMS%CV_NGI_SHORT), VOL_FRA_GI_DX_ALL( NDIM, NPHASE, GIDIMS%CV_NGI_SHORT) )
         ALLOCATE( SLOC_VOL_FRA(NPHASE, CV_SNLOC), SLOC2_VOL_FRA(NPHASE, CV_SNLOC))
         ALLOCATE( SVOL_FRA(NPHASE, GIDIMS%SBCVNGI), SVOL_FRA2(NPHASE, GIDIMS%SBCVNGI) )
      ...
      IF(RETRIEVE_SOLID_CTY) THEN
         ALLOCATE( SIGMAGI_STAB_SOLID_RHS( NDIM_VEL * NPHASE, NDIM_VEL * NPHASE, GIDIMS%CV_NGI ))
      ...
      ALLOCATE( UDIFF_SUF_STAB( NDIM_VEL, NDIM, NDIM, NPHASE, GIDIMS%SBCVNGI ) )
      UDIFF_SUF_STAB = 0.0
      ...
      ALLOCATE( CVFENX_ALL_REVERSED(NDIM,GIDIMS%CV_NGI,CV_NLOC), UFENX_ALL_REVERSED(NDIM,GIDIMS%CV_NGI,U_NLOC) ) ! NOT CALCULATED IN SUB cv_fem_shape_funs_plus_storage

      ALLOCATE( UFEN_REVERSED(GIDIMS%CV_NGI,U_NLOC), CVFEN_SHORT_REVERSED(GIDIMS%CV_NGI,CV_NLOC) )
      ALLOCATE( CVN_SHORT_REVERSED(GIDIMS%CV_NGI,CV_NLOC), CVN_REVERSED(GIDIMS%CV_NGI,CV_NLOC), CVFEN_REVERSED(GIDIMS%CV_NGI,CV_NLOC) )
      ALLOCATE( SBCVFEN_REVERSED(GIDIMS%SBCVNGI,CV_SNLOC), SBUFEN_REVERSED(GIDIMS%SBCVNGI,U_SNLOC) )
      ...
      ! Storage for pointers to the other side of the element.
      ALLOCATE( STORED_U_ILOC_OTHER_SIDE( U_SNLOC, GIDIMS%NFACE, TOTELE*ISTORED_OTHER_SIDE ) )
      ALLOCATE( STORED_U_OTHER_LOC( U_NLOC, GIDIMS%NFACE, TOTELE*ISTORED_OTHER_SIDE ) )
      ALLOCATE( STORED_MAT_OTHER_LOC( MAT_NLOC, GIDIMS%NFACE, TOTELE*ISTORED_OTHER_SIDE ) )

      ALLOCATE( POSINMAT_C_STORE( U_NLOC,P_NLOC, TOTELE*IDO_STORE_AC_SPAR_PT) )
      ALLOCATE( POSINMAT_C_STORE_SUF_DG( U_SNLOC,P_SNLOC,GIDIMS%NFACE,TOTELE*IDO_STORE_AC_SPAR_PT ) )

      ALLOCATE( FACE_ELE( GIDIMS%NFACE, TOTELE ) )
      ! Calculate FACE_ELE
      ...
      IF( GOT_DIFFUS ) THEN
         CALL DG_DERIVS_ALL( U_ALL, UOLD_ALL, &
              ...
              GIDIMS%CV_NGI, U_NLOC, CVWEIGHT, &
              UFEN, UFENLX_ALL(1,:,:), UFENLX_ALL(2,:,:), UFENLX_ALL(3,:,:), &
              CVFEN, CVFENLX_ALL(1,:,:), CVFENLX_ALL(2,:,:), CVFENLX_ALL(3,:,:), &
              X_NONODS, X, Y, Z, &
              GIDIMS%NFACE, FACE_ELE, U_SLOCLIST, CV_SLOCLIST, STOTEL, U_SNLOC, CV_SNLOC, WIC_U_BC_ALL_VISC, SUF_U_BC_ALL_VISC, &
              GIDIMS%SBCVNGI, SBUFEN, SBUFENSLX, SBUFENSLY, SBCVFEWEIGH, &
      ...
      if( RETRIEVE_SOLID_CTY ) THEN
         ...
         allocate( vol_s_gi( GIdims%cv_ngi_short ) )
         allocate( vol_s_min_gi( GIdims%cv_ngi_short ) )
         allocate( cv_dengi( nphase, GIdims%cv_ngi_short ) )
         ...
         if (.not.is_porous_media) then
            ...
            Loop_DGNods1: DO U_ILOC = 1, U_NLOC
               ...
               Loop_DGNods2: DO U_JLOC = 1, U_NLOC
                  ...
                  Loop_Gauss2: DO GI = 1, GIDIMS%CV_NGI_SHORT
      ...
      Loop_Elements2: DO ELE = 1, TOTELE
         ...
         Between_Elements_And_Boundary: DO IFACE = 1, GIDIMS%NFACE
            ...
            CALL DGSDETNXLOC2_ALL( CV_SNLOC, GIDIMS%SBCVNGI, NDIM, &
                 XSL_ALL, &
            ...
            IF(GOT_DIFFUS .AND. LINEAR_HIGHORDER_DIFFUSION) THEN
               IF(ELE2.GT.0) THEN
                  CALL DG_VISC_LIN( S_INV_NNX_MAT12, NNX_MAT_ELE(:,:,:,ELE), NNX_MAT_ELE(:,:,:,ELE2), NN_MAT_ELE(:,:,ELE), NN_MAT_ELE(:,:,ELE2),  &
                       U_SNLOC, U_NLOC, SBUFEN_REVERSED, SDETWE, GIDIMS%SBCVNGI, SNORMXN_ALL, NDIM, &
            ...
            if (capillary_pressure_activated.and..not. Diffusive_cap_only) call Introduce_Cap_press_term(&
                 state, packed_state,storage_state, Mdims, X_ALL, LOC_U_RHS, ele, x_nloc,FACE_ELE,cv_ndgln, cv_nloc, cv_snloc, u_snloc, &
            ...
            If_on_boundary_domain: IF(SELE /= 0) THEN
               ...
               Loop_ILOC2: DO U_SILOC = 1, U_SNLOC
                  ...
                  Loop_JLOC2: DO P_SJLOC = 1, P_SNLOC
                     ...
                     if (GET_C_IN_CV_IN_DYNCORE) then                                                   !WE NEED A REVERSED SBCVN
                        NMX_ALL = matmul(SNORMXN_ALL( :, : ), SBUFEN_REVERSED( :, U_SILOC ) * SBCVN( P_SJLOC, : ) * SDETWE( : ))
                     else
               ...
               if ( got_free_surf ) then
                  if ( symmetric_P ) then ! us this if not the other formation of MASS_SUF in cv_adv_dif
                     if ( WIC_P_BC_ALL( 1,1,SELE ) == WIC_P_BC_FREE ) then ! on the free surface...
                        DO P_SILOC = 1, P_SNLOC
                           P_ILOC = CV_SLOC2LOC( P_SILOC )
                           P_INOD=CV_NDGLN( (ELE-1)*CV_NLOC + P_ILOC )
                     ...
                     DO P_SJLOC = 1, CV_SNLOC
                        ...
                        DO U_SILOC = 1, U_SNLOC
                           ...
                           DO SGI = 1, GIDIMS%SBCVNGI
                              ...
                              if (GET_C_IN_CV_IN_DYNCORE) then                                       !WE NEED A REVERSED SBCVN
                                 RNN = SDETWE( SGI ) * SBUFEN_REVERSED( SGI, U_SILOC ) * SBCVN( P_SJLOC, SGI )
                           ...
                           IF(DISC_PRES) CALL USE_POSINMAT_C_STORE_SUF_DG( COUNT2, U_INOD, P_JNOD2,  &
                                ...
                                TOTELE, GIDIMS%NFACE, U_SNLOC, P_SNLOC )
                              ...
                              IF ( .NOT.GOT_C_MATRIX ) THEN
                                 DO IDIM = 1, NDIM_VEL
                                    ...
                                    IF(GET_C_IN_CV_IN_DYNCORE) THEN
                                       IF (DISC_PRES) THEN
                                          C( IDIM, IPHASE, COUNT ) = C( IDIM, IPHASE, COUNT ) &
                                               + VNMX_ALL( IDIM ) * SELE_OVERLAP_SCALE( P_JLOC ) * 0.5
            ...
            ELSE
               ...
               DO IDIM = 1, NDIM
                  ...
                  AVE_SNORMXN_ALL(IDIM) = SUM( SNORMXN_ALL( IDIM, : )) / REAL(GIDIMS%SBCVNGI)
               END DO
            ...
            If_diffusion_or_momentum3: IF(GOT_DIFFUS .OR. GOT_UDEN) THEN
               ...
               If_GOT_DIFFUS2: IF(GOT_DIFFUS.and.LINEAR_HIGHORDER_DIFFUSION) THEN
                  ...
                  CALL LINEAR_HIGH_DIFFUS_CAL_COEFF_STRESS_OR_TENSOR( STRESS_IJ_ELE_EXT, S_INV_NNX_MAT12,  &
                       ...
                       SBUFEN_REVERSED,SBCVFEN_REVERSED,SDETWE, GIDIMS%SBCVNGI, NDIM, SLOC_UDIFFUSION, SLOC_UDIFFUSION_VOL, SLOC2_UDIFFUSION, SLOC2_UDIFFUSION_VOL, UDIFF_SUF_STAB, &
               ...
               ELSE IF(GOT_DIFFUS) THEN If_GOT_DIFFUS2
                  ...
                  CALL DIFFUS_CAL_COEFF_STRESS_OR_TENSOR( DIFF_COEF_DIVDX, &
                       ...
                       U_SNLOC, U_NLOC, CV_SNLOC, CV_NLOC, MAT_NLOC, NPHASE, &
                       SBUFEN_REVERSED,SBCVFEN_REVERSED,GIDIMS%SBCVNGI, NDIM_VEL, NDIM, SLOC_UDIFFUSION, SLOC_UDIFFUSION_VOL, SLOC2_UDIFFUSION, SLOC2_UDIFFUSION_VOL, UDIFF_SUF_STAB, &
                       HDC, &
