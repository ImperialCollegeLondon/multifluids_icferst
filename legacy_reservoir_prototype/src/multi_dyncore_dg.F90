    ...
    do cv_nod = 1, size(multi_absorp%Magma%val,4)
        ...
        IF (.not. ( JUST_BL_DIAG_MAT .OR. Mmat%NO_MATRIX_STORE ) ) then
          if(block_mom) then
            ...
            Mmat%DGM_PETSC = allocate_momentum_block_matrix(sparsity,velocity,mspars%ele%fin, mspars%ele%col, big_block)
          else
...
!
      ...
      contains
        ...
        subroutine solve_and_update_velocity(Mmat,Velocity, CDP_tensor, U_RHS, diagonal_A,block)
          ...
          if(block) then
            !print* ,Mmat%DGM_PETSC%row_numbering%nprivatenodes
            ...
            call petsc_solve( packed_vel, Mmat%DGM_PETSC, RHS, option_path = trim(solver_option_velocity))
