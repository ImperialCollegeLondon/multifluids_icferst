                    ...
                    IF ( GOT_DIFFUS ) THEN
                        DO IPHASE = 1, Mdims%nphase
                            ...
                            ELSE
                                ...
                                if ( UDIFFUSION_VOL%have_field ) SLOC_UDIFFUSION_VOL( IPHASE, CV_SILOC ) = UDIFFUSION_VOL%val(1,1, IPHASE, MAT_INOD ) !UDIFFUSION_VOL%val( IPHASE, 1,1,MAT_INOD )
                                SLOC2_UDIFFUSION( 1:Mdims%ndim, 1:Mdims%ndim, IPHASE, CV_SILOC ) = UDIFFUSION( 1:Mdims%ndim, 1:Mdims%ndim, IPHASE, MAT_INOD2 )
                                if ( UDIFFUSION_VOL%have_field ) SLOC2_UDIFFUSION_VOL( IPHASE, CV_SILOC ) = UDIFFUSION_VOL%val(1,1, IPHASE, MAT_INOD2 ) !UDIFFUSION_VOL%val( IPHASE, 1,1,MAT_INOD2 )
                            ENDIF
                        END DO
                    END IF
                END DO
