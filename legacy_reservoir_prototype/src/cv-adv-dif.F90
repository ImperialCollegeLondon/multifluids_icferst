...
! FOR SUB CALC_LIMIT_MATRIX_MAX_MIN:
      ...
      ELSE ! endof IF ( IANISOLIM == 0 ) THEN
         ...
         CALL CALC_ANISOTROP_LIM( &
              ...
              packed_state, "anisotrop", storageindexes(2))
      ...
      IF ( GOT_DIFFUS ) THEN
         CALL DG_DERIVS_ALL2( FEMT_ALL, FEMTOLD_ALL, &
              ...
              packed_state, "DGDEVAL2", StorageIndexes( 3 ) )
...
!            IF ( IGOT_T2 == 1 ) THEN
      ...
      Loop_Elements: DO ELE = 1, TOTELE
         ...
         CALL DETNLXR_INVJAC_PLUS_STORAGE( ELE, X_ALL, X_NDGLN, TOTELE, X_NONODS, &
              ...
              SCVFENX_ALL, &
              NDIM, INV_JAC, packed_state, "INVJAC", StorageIndexes(4) )
...
! local variables...
             ...
             SUBROUTINE UNPACK_LOC( LOC_F, T_ALL, NPHASE, NFIELD, IPT, STORE, IGOT_T_PACK, GLOBAL_FACE, IGOT_T_CONST, IGOT_T_CONST_VALUE,&
                TOTAL_GLOBAL_FACE, packed_state, StorName, indx )
...
! local variables...
             ...
             DO IPHASE=1,NPHASE
                ...
                if (indx < 0) then
                    if ((IPHASE+(GLOBAL_FACE-1)*NPHASE)<int(packed_state%scalar_fields(abs(indx))%ptr%val(TOTAL_GLOBAL_FACE*NPHASE+1))) then
                        ...
                        packed_state%scalar_fields(abs(indx))%ptr%val(TOTAL_GLOBAL_FACE*NPHASE+1) = -1.0
                     ...
                     if (.not.IGOT_T_CONST(IPHASE)) then
                         IF(STORE) THEN ! Put in storage if not already in storage...
                             if (indx < 0) then!We may need to store a new value
                                 ...
                                 packed_state%scalar_fields(abs(indx))%ptr%val(IPHASE+(GLOBAL_FACE-1)*NPHASE) = LOC_F(IPT-1)
                                 !Store input index to check when we start to get data instead of sting data
                                 packed_state%scalar_fields(abs(indx))%ptr%val(TOTAL_GLOBAL_FACE*NPHASE+1) = IPHASE+(GLOBAL_FACE-1)*NPHASE
                             else if (GLOBAL_FACE==1 .and. indx == 0) then !The first time we need to introduce the targets in state
                                 if (has_scalar_field(packed_state, "Fld"//StorName)) then
                                     !If we are recalculating due to a mesh modification then
                                     !we return to the original situation
                                     call remove_scalar_field(packed_state, "Fld"//StorName)
                                 end if
                                 ...
                                 call insert(packed_state, targ_fieldToStore, "Fld"//StorName)
...
!
                          ...
                          T_ALL(IPHASE) = packed_state%scalar_fields(abs(indx))%ptr%val(IPHASE+(GLOBAL_FACE-1)*NPHASE)
                         end if
                     ENDIF
...
! The below does not seem superefficient in terms of updating halo information. 
  ...
  SUBROUTINE DG_DERIVS_UVW( U, UOLD, V, VOLD, W, WOLD, &
       ...
       SBCVFEN, SBCVFENSLX, SBCVFENSLY, packed_state, StorName, Indexes)
    ...
    CALL DG_DERIVS( U, UOLD, &
         ...
         WIC_U_BC_DIRICHLET, SBCVNGI, SBUFEN, SBUFENSLX, SBUFENSLY, SBWEIGH, &
         SBCVFEN, SBCVFENSLX, SBCVFENSLY,&
         packed_state, StorName//"U", Indexes(1))
    IF(NDIM_VEL.GE.2) THEN
       CALL DG_DERIVS( V, VOLD, &
            ...
            WIC_U_BC_DIRICHLET, SBCVNGI, SBUFEN, SBUFENSLX, SBUFENSLY, SBWEIGH, &
            SBCVFEN, SBCVFENSLX, SBCVFENSLY,&
            packed_state, StorName//"V", Indexes(2))
    ELSE
    ...
    IF(NDIM_VEL.GE.3) THEN
       CALL DG_DERIVS( W, WOLD, &
            ...
            WIC_U_BC_DIRICHLET, SBCVNGI, SBUFEN, SBUFENSLX, SBUFENSLY, SBWEIGH, &
            SBCVFEN, SBCVFENSLX, SBCVFENSLY,&
            packed_state,StorName//"W", Indexes(3))
    ELSE
  ...
  SUBROUTINE DG_DERIVS_ALL2( FEMT, FEMTOLD, &
       ...
       SBCVNGI, SBCVFEN, SBCVFENSLX, SBCVFENSLY, SBWEIGH, &
       X_SBCVFEN, X_SBCVFENSLX, X_SBCVFENSLY,&
       packed_state, StorName, StorageIndexes  )
  ...
  SUBROUTINE CALC_ANISOTROP_LIM_VALS2( &
    ...
    IF( STORE_ELE ) THEN
       ...
       CALL FINPTSSTORE(T_ALL,FEMT_ALL,USE_FEMT,NFIELD,NONODS,NLOC,NGI,TOTELE,NDGLNO, &
            ...
            BOUND, REFLECT, packed_state, storName, indx)
...
!    NORMZ1=0.0
    IF(REFLECT) THEN
       ...
       DO ELE=1,TOTELE! Was loop
           ...
           NX_ALL, packed_state, StorName , indx)
