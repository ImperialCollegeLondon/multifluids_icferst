...
contains
  ...
  integer PURE  function my_size_real(A,n)
    ...
    SUBROUTINE CV_ASSEMB( state, packed_state, storage_state, &
         ...
         saturation,OvRelax_param, Phase_with_Pc, indx, Storname, IDs_ndgln, Courant_number, RECALC_C_CV, SUF_INT_MASS_MATRIX, MASS_P_CV)
      ...
      REAL, DIMENSION( :, :, : ), intent( inout ), allocatable :: GAMMA_PRES_ABS, GAMMA_PRES_ABS_NANO, INV_B ! (nphase, nphase, cv_nonods)
...
!
      ALLOCATE( NUGI_ALL( NDIM, NPHASE ),  NUOLDGI_ALL(NDIM, NPHASE), NU_LEV_GI( NDIM, NPHASE ) )
!
...
! NFIELD Variables:
      ALLOCATE(CAP_DIFF_COEF_DIVDX(NPHASE), DIFF_COEF_DIVDX(NPHASE), DIFF_COEFOLD_DIVDX(NPHASE), &
               NDOTQNEW(NPHASE), LIMT2OLD(NPHASE), LIMDTOLD(NPHASE), INCOMEOLD(NPHASE), NDOTQOLD(NPHASE), &
...
!      print *,'after allocate'
      ...
      IF ( NPRES > 1 ) THEN
         ...
         ALLOCATE( SIGMA_INV_APPROX( NPHASE, CV_NONODS ), SIGMA_INV_APPROX_NANO( NPHASE, CV_NONODS ), N( CV_NONODS ) )
...
! This determins how implicit velocity is in the cty eqn.
                     ...
                     Conditional_GETCT2: IF ( GETCT ) THEN ! Obtain the CV discretised CT eqations plus RHS
                        ...
                        CALL PUT_IN_CT_RHS( CT, C, GET_C_IN_CV_ADVDIF_AND_CALC_C_CV, ct_rhs_phase_cv_nodi, ct_rhs_phase_cv_nodj, U_NLOC, U_SNLOC, SCVNGI, GI, NCOLCT, NDIM, &
                             CV_NONODS, U_NONODS, NPHASE, between_elements, on_domain_boundary, ELE, ELE2, HDC, &
                             ...
                             SUF_INT_MASS_MATRIX2, MASS_P_CV )
...
! this needs to be corrected (its correct but misleading)...
                          ROBIN1(:) = SUF_T_BC_ROB1_ALL(1,:, CV_SILOC+CV_SNLOC*(sele-1))
                          ROBIN2(:) = SUF_T_BC_ROB2_ALL(1,:, CV_SILOC+CV_SNLOC*(sele-1))
                       end where
                     END IF
...
!    ENDIF
  ...
  SUBROUTINE PUT_IN_CT_RHS( CT, C, GET_C_IN_CV_ADVDIF_AND_CALC_C_CV, ct_rhs_phase_cv_nodi, ct_rhs_phase_cv_nodj, U_NLOC, U_SNLOC, SCVNGI, GI, NCOLCT, NDIM, &
       ...
       ELE, ELE2, HDC, JCOUNT_KLOC, JCOUNT_KLOC2, ICOUNT_KLOC, ICOUNT_KLOC2, C_JCOUNT_KLOC, C_JCOUNT_KLOC2, C_ICOUNT_KLOC, C_ICOUNT_KLOC2, U_OTHER_LOC, U_SLOC2LOC,   &
       SUFEN, SCVDETWEI, CVNORMX_ALL, DEN_ALL, CV_NODI, CV_NODJ, &
       UGI_COEF_ELE_ALL,  &
       UGI_COEF_ELE2_ALL,  &
...
! local memory sent down for speed...
       UDGI_IMP_ALL,     RCON, RCON_J, NDOTQ_IMP, rcon_in_ct, rcon_j_in_ct,    UDGI_ALL, UOLDDGI_ALL, UDGI_HAT_ALL, CAP_DIFF_COEF_DIVDX, SUF_INT_MASS_MATRIX, MASS_P_CV)
...
!    LOGICAL, PARAMETER :: more_in_ct=.false.
    ...
    LOGICAL, intent( in ) :: integrate_other_side_and_not_boundary, RETRIEVE_SOLID_CTY, between_elements, on_domain_boundary, GET_C_IN_CV_ADVDIF_AND_CALC_C_CV, SUF_INT_MASS_MATRIX
...
! Need to pass down cap_diff_coef_divdx to this routine
    ...
    INTEGER :: U_KLOC, U_KLOC2, JCOUNT_IPHA, IDIM, U_NODK, U_NODK_IPHA, JCOUNT2_IPHA, &
         U_KLOC_LEV, U_NLOC_LEV, IPHASE, U_SKLOC, I, J, U_KKLOC
...
!     if(more_in_ct) then
!       rcon_in_ct(:)=SCVDETWEI( GI ) * SUFEN( U_KLOC, GI ) / real(nphase)
!     else
       rcon_in_ct(:)=rcon(:)
!     endif
...
!     if(more_in_ct) then
!       rcon_j_in_ct(:)=SCVDETWEI( GI ) * SUFEN( U_KLOC, GI ) / real(nphase)
!     else
       rcon_j_in_ct(:)=rcon_j(:)
!     endif
...
!
  ...
  PURE SUBROUTINE GET_INT_VEL_ORIG_NEW( NPHASE, NDOTQNEW, NDOTQ, INCOME, &
       GI, SUFEN, U_NLOC,&
    ...
    ! Calculate NDOTQ and INCOME on the CV boundary at quadrature pt GI.
    IMPLICIT NONE
    INTEGER, intent( in ) :: NPHASE, GI, U_NLOC, &
         CV_DG_VEL_INT_OPT, ELE, ELE2, &
         SELE, U_SNLOC, &
         NDIM
    LOGICAL, intent( in ) :: between_elements, on_domain_boundary
    ...
    REAL, DIMENSION( :, :, : ), intent( in ) :: LOC_NU, LOC2_NU, LOC_U, LOC2_U
    REAL, DIMENSION( :, : ), intent( in ) :: CVNORMX_ALL
...
!             ENDIF
  ...
  END SUBROUTINE GET_INT_VEL_ORIG_NEW
      ...
      SUBROUTINE GET_INT_VEL_POROUS_VEL( NPHASE, NDOTQNEW, NDOTQ,INCOME, &
       ...
       SUF_SIG_DIAGTEN_BC, &
       UGI_COEF_ELE_ALL, UGI_COEF_ELE2_ALL, &
       CV_SLOC2LOC, CV_SNLOC, CV_ILOC, SCVFEN, &
    ...
    INTEGER, DIMENSION( : ), intent( in ) :: U_SLOC2LOC, u_other_loc
    INTEGER, DIMENSION( :, :, : ), intent( in ) :: WIC_U_BC_ALL
    REAL, DIMENSION( :, :  ), intent( in ) :: SUFEN
...
!                  IF (.false.) THEN !<= this one for strong boundary conditions
  ...
  END SUBROUTINE MOD_1D_FORCE_BAL_C
    ...
    SUBROUTINE DEF_SIGMA_PIPE_FRICTION(SIGMA, U, DIAM, IPHASE)
    IMPLICIT NONE
    REAL, INTENT( IN ) :: U, DIAM
    INTEGER, INTENT( IN ) :: IPHASE
    REAL, INTENT( INOUT ) :: SIGMA

! Local variables
    REAL :: E_ROUGHNESS, DEFAULT_DEN(2), DEFAULT_VISC(2)

    E_ROUGHNESS = 1.E-6 ! Pipe roughness.
    DEFAULT_DEN(1) = 1000.0 ! Density of water
    DEFAULT_DEN(2) = 900.0 ! Density of oil
    DEFAULT_VISC(1) = 1.E-3 ! Viscocity of water
    DEFAULT_VISC(2) = 1.E-2 ! Viscocity of oil

    CALL SIGMA_PIPE_FRICTION(SIGMA,U,DIAM,DEFAULT_DEN(IPHASE),DEFAULT_VISC(IPHASE),E_ROUGHNESS)
    RETURN
    END SUBROUTINE DEF_SIGMA_PIPE_FRICTION
