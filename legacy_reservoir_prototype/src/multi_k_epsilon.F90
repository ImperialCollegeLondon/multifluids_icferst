...
subroutine keps_advdif_diagnostics(state)
  ...
  !!-PY test it
  print *, 'using the k_epsilon model'
  ...
  call keps_calculate_rhs(state)              !!-PY this is the problem
...
!print *, '180420162Soure_term:', src%val
...
!print *, 'ele_count(fields(1))', ele_count(fields(1))
...
!!-PY this is problem
!print *, 'dg_velocity', dg_velocity
!print *, 'element_neighbour_owned', element_neighbour_owned(u, ele)
!print *, 'element_owned', element_owned(u, ele)
...
!print *, '180420163Soure_term:', src%val

!!-PY this is the problem  src_abs_terms(1)
!print *, '180420164src_abs_terms1:', src_abs_terms(1)%val
!print *, '180420164src_abs_terms2:', src_abs_terms(2)%val
!print *, '180420164src_abs_terms3:', src_abs_terms(3)%val
           call addto(src, src_abs_terms(term))

!print *, '18042016Soure_term:', src%val
...
!!-PY changed it for k_epsilon model
  ...
  real, dimension(ele_ngi(k, ele)) :: scalar, c_eps_3
...
!!-PY this is the problem
  ...
  nodes_u = ele_nodes(u, ele)



  shape_p => ele_shape(k, ele)
  nodes_p = ele_nodes(k, ele)
...
!  call transform_to_physical( X, ele, shape, dshape=dshape, detwei=detwei )
   call transform_to_physical( X, ele, shape_u, dshape=dshape_u, detwei=detwei )
...
!     print *, 'Compute grad_u'
!     print *, 'grad_u size', size(grad_u)
!     print *, 'grad_u', grad_u
!     print *, 'u size', size(u%val)
!     print *, 'u', u%val
!    print *, 'shape%n size', size(shape%n)
!     print *, 'shape%n', shape%n
!     print *, 'X size', size(X%val)
!     print *, 'X', X%val
!     print *, 'bc_value size', size(bc_value%val)
!     print *, 'bc_value', bc_value%val
...
!stop 13042016
     ...
     grad_u = dg_ele_grad_at_quad(u, ele, shape_u, X, bc_value, bc_type)   !!-PY this is the problem
...
!print *, 'Compute scalar_eddy_visc_ele'
...
!print *, 'Compute P'
  ...
  rhs_addto(1,:) = shape_rhs(shape_p, detwei*rhs)
...
!print *, 'Compute A'
  ...
  rhs_addto(2,:) = shape_rhs(shape_p, detwei*rhs)
...
!print *, 'Compute Gk'
  ...
  if(have_buoyancy_turbulence) then    
    ...
    if(.not.(buoyancy_density%mesh == k%mesh)) then
       ...
       call transform_to_physical( X, ele, shape_density, dshape=dshape_u ) 
    else
       dshape_density = dshape_u
    end if


!!-Py there is a problem for p1dgp2
...
!print *, 'vector_size', size(vector)
!print *,  'vector', vector  

!print *,  'ele_grad_at_quad(buoyancy_density, ele, dshape_density)', ele_grad_at_quad(buoyancy_density, ele, dshape_u)
!print *,  'ele_val_at_quad(g, ele)', ele_val_at_quad(g, ele)

    
    !vector = ele_val_at_quad(g, ele)*1.0 !!ele_grad_at_quad(buoyancy_density, ele, dshape_density)
    
    ! multiply vector component by scalar and sum across dimensions - note that the5
    ...
    !!-PY add it
    !!-PY the scalar is the problem
!print *, 'scalar_size', size(scalar)
!print *, 'scalar', scalar
!stop 21042016
!    scalar=1.0


    rhs_addto(3,:) = shape_rhs(shape_p, scalar * detwei)
  ...
  if(continuity(k)<0) then
     invmass = inverse(shape_shape(shape_p, shape_p, detwei))
     do term = 1, 3
...
!print *, 'rhs_addto(1,:)_szie', size(rhs_addto)
!print *, 'rhs_addto(1,:)', rhs_addto(1,:)
!print *, 'rhs_addto(2,:)', rhs_addto(2,:)
!print *, 'rhs_addto(3,:)', rhs_addto(3,:)

!stop 55555

  do term = 1, 3
!print *, '18062016_py1'
     call addto(src_abs_terms(term), nodes_p, rhs_addto(term,:))
!print *, 'nodes_p', nodes_p
!print *, 'src_abs_terms(term)_size', size(src_abs_terms(:))
!print *, 'src_abs_terms(term)', src_abs_terms(term)%val
...
!!   call print_state(state)  
  ...
  if (have_visc) then
!!-PY-This is the problem
!print *,'viscosity_size',  size(viscosity%val(1,1,:)), size(eddy_visc%val(1,1,:))
