...
contains
    ...
    subroutine destroy_multi_matrices(Mmat)
        ...
        if (associated(Mmat%CV_RHS%val)) call deallocate(Mmat%CV_RHS)!<=This one seems to give problems to PY
!        if (associated(Mmat%CV_RHS%val)) deallocate(Mmat%CV_RHS%val)!if that is the case, at least use this line instead
