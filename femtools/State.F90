...
module state_module
  ...
  type(petsc_csr_matrix), save, target :: fake_petsc_csr_matrix
...
contains
  ...
  subroutine deallocate_state(state)
    ...
    if (associated(state%petsc_csr_matrix_names)) then
       deallocate(state%petsc_csr_matrix_names)
    end if
    ...
    if (associated(state%petsc_csr_matrices)) then
       do i=1,size(state%petsc_csr_matrices)
          call deallocate(state%petsc_csr_matrices(i)%ptr)
          deallocate(state%petsc_csr_matrices(i)%ptr)
       end do
       deallocate(state%petsc_csr_matrices)
    end if
  ...
  subroutine insert_petsc_csr_matrix(state, matrix, name)
    !!< Insert a block matrix into state.
    type(state_type), intent(inout) :: state
    ...
    if (.not.associated(state%petsc_csr_matrices)) then
       ! Special case first entry.
       ...
       do i=1,size(state%petsc_csr_matrices)
          if (trim(name)==trim(state%petsc_csr_matrix_names(i))) then
             ! The name is present!
             call deallocate(state%petsc_csr_matrices(i)%ptr)
             state%petsc_csr_matrices(i)%ptr = matrix
             !call incref(matrix)
             call incref_petsc_csr_matrix(matrix)
             return
          end if
       end do

       ! If we get to here then this is a new matrix.
       tmp_petsc_csr_matrices=>state%petsc_csr_matrices
       tmp_names=>state%petsc_csr_matrix_names

       old_size=size(tmp_petsc_csr_matrices)
       ...
       state%petsc_csr_matrix_names(1:old_size)= tmp_names

       state%petsc_csr_matrices(old_size+1)%ptr = matrix
       state%petsc_csr_matrix_names(old_size+1) = name

       deallocate(tmp_names)
       deallocate(tmp_petsc_csr_matrices)
  ...
  subroutine insert_and_alias_petsc_csr_matrix(state, matrix, name)
    ...
    p_matrix=extract_petsc_csr_matrix(state(1), trim(name))
  ...
  type(petsc_csr_matrix_pointer), dimension(:), pointer :: tmp_fields
  character(len=FIELD_NAME_LEN), dimension(:), pointer :: tmp_names
  integer :: idx, i, j, old_size

  if (.not. has_petsc_csr_matrix(state, name)) then
    if (present(stat)) then
      stat=1
    else
       ewrite(-1,*) "State: "//trim(state%name)
  ...
  tmp_fields=>state%petsc_csr_matrices
  tmp_names=>state%petsc_csr_matrix_names
  ...
  function extract_from_one_petsc_csr_matrix(state, name, stat) result (matrix)
    ...
    if (present(stat)) stat=0
    matrix => fake_petsc_csr_matrix

    if (associated(state%petsc_csr_matrices)) then
       do i=1,size(state%petsc_csr_matrices)
          if (trim(name)==trim(state%petsc_csr_matrix_names(i))) then
             ! Found the right field
             matrix=>state%petsc_csr_matrices(i)%ptr
             return
          end if
       end do
    end if
  ...
  function extract_from_any_petsc_csr_matrix(state, name, stat) result (matrix)
    ...
    do i = 1, size(state)
      if (associated(state(i)%petsc_csr_matrices)) then
         do j=1,size(state(i)%petsc_csr_matrices)
            if (trim(name)==trim(state(i)%petsc_csr_matrix_names(j))) then
               ! Found the right field
               matrix=>state(i)%petsc_csr_matrices(j)%ptr
               return
            end if
         end do
      end if
  ...
  function extract_petsc_csr_matrix_by_index(state, index) result (field)
    ...
    assert(index<=petsc_csr_matrix_count(state))

    field=>state%petsc_csr_matrices(index)%ptr

  end function extract_petsc_csr_matrix_by_index
  ...
  pure function petsc_csr_matrix_count(state)
    !!< Return the number of csr_matrices in state.
