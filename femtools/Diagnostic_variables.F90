...
& PYTHON_FUNC_LEN, integer_size, real_size
  ...
  type :: stat_type
   ...
   !! Are we continuing from a detector checkpoint file?
    ...
    integer(kind = c_long) :: non_linear_iterations
  end type stat_type
...
contains
  ...
  subroutine initialise_diagnostics(filename, state, ICFERST)
    !!< Set up the diagnostic file headers.

    character(len=*) :: filename
    type(state_type), dimension(:), intent(in) :: state
    logical, optional, intent(in) :: ICFERST
    ...
    if(getprocno() == 1) then
      ...
      write(default_stat%diag_unit, '(a)') trim(buffer)
      if (present_and_true(ICFERST)) then
        column=column+1
        buffer=field_tag(name="EqNonLinearIterations", column=column, statistic="value")
        write(default_stat%diag_unit, '(a)') trim(buffer)
      end if
