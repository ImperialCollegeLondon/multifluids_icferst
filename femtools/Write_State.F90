...
module write_state_module
  ...
  public :: initialise_write_state, do_write_state, write_state, write_state_mindt, write_state_module_check_options, &
...
contains
  ...
  subroutine write_state_mindt(dump_no, acctim, state)
    !!< Data output routine. Write output data.

    integer, intent(inout) :: dump_no
    type(state_type), dimension(:), intent(inout) :: state

    character(len = OPTION_PATH_LEN) :: dump_filename, dump_format, dump_filename_mindt
    integer :: max_dump_no, stat
    ...
    call get_option("/simulation_name", dump_filename)
    call get_option("/io/max_dump_file_count", max_dump_no, stat, default = huge(0))
    write(acctim_str , '(f14.8)') acctim
    dump_filename_mindt =  trim(dump_filename)//'_mindt_'//trim(acctim_str)
    dump_no = modulo(dump_no, max_dump_no)
    ...
    select case(trim(dump_format))
      case("vtk")
         ewrite(2, *) "Writing output " // int2str(dump_no) // " to vtu"
         call vtk_write_state_new_options(dump_filename_mindt, dump_no, state)
      case default
        FLAbort("Unrecognised dump file format.")
    end select
