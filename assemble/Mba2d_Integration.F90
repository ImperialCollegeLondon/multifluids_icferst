...
!#define DUMP_HALO
    ...
    allocate(solid_nodes_to_lock(node_count(input_positions))); solid_nodes_to_lock = 0;
    ! if (have_option('/solid_implicit')) then
    ...
    !     found_now = input_positions%mesh%region_ids(i) == 2
    !     if (found_now) then 
    !       do j = 1, nloc ! (=3)
