...
module zoltan_integration
  ...
  subroutine setup_module_variables(states, final_adapt_iteration, zz, flredecomp, mesh_name)
    ...
    if (have_option(trim(zoltan_global_base_option_path) // "/element_quality_cutoff")) then
       ...
       if (have_option( '/material_phase[0]/scalar_field::Pressure/prognostic/CV_P_matrix' )) Then
         !CV Pressure is much more robust so we relax this limitation for mesh adaptivity
         zoltan_global_quality_tolerance = 0.05
       else
