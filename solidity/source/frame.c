...
void CHRcpynoext(CHR *c1, CHR *c2)  /* copy no extension */
{ 
  INT i;
  i=0;
...
DBL ***TalDBL3(INT m3, INT m2, INT m1)
{ 
  INT isize,i2,i3;
  DBL     *p1b;
  DBL    **p2b,  **p2e, **p2;
...
INT ***TalINT3(INT m3, INT m2, INT m1)
{ 
  INT isize,i2,i3;
  INT     *p1b;
  INT    **p2b,  **p2e, **p2;
...
INT Getname(INT argc, char **argv, CHR *name)
{ 
  INT i;
  for(i=0;i<argc;i++)
  { 
...
void TreadDBL1(FILE *fptr, INT n1, DBL *d1aray) 
{ 
  INT i;
  DBL dnum;
  for(i=0;i<n1;i++)
...
void TreadDBL2(FILE *fptr, INT n1, INT n2, DBL **d2aray) 
{ 
  INT i1,i2;
  DBL dnum;
  for(i1=0;i1<n1;i1++)
...
void TreadINT1(FILE *fptr, INT n1, INT *i1aray) 
{ 
  INT i; INT inum;
  for(i=0;i<n1;i++)
  { 
...
void TreadINT2(FILE *fptr, INT n1, INT n2, INT **i2aray) 
{ 
  INT i1,i2;
  INT inum;
  for(i1=0;i1<n1;i1++)
...
void TformDBL1(FILE *fptr, DBL dinit, INT m1, DBL **d1aray) 
{ 
  INT i1,n1;
  DBL dnum;
  DBL *d1=*d1aray;
...
void TformDBL2(FILE *fptr, DBL dinit, INT m1, INT m2, DBL ***d2aray) 
{ 
  INT i1,i2,n1,n2,how;
  DBL dnum;
  DBL **d2=*d2aray;
...
void TformDBL2_inv(FILE *fptr, DBL dinit, INT m1, INT m2, DBL ***d2aray) 
{ 
  INT i1,i2,n1,n2,how;
  DBL dnum;
  DBL **d2=*d2aray;
...
void TformDBL3(FILE *fptr, DBL dinit, INT m1, INT m2, INT m3, DBL ****d3aray) 
{ 
...
/* transposed a bit: [i][j][e] => [e][i][j]
*/
void TformDBL3_inv(FILE *fptr, DBL dinit, INT m1, INT m2, INT m3, DBL ****d3aray) 
{ 
  INT i1,i2,i3,n1,n2,n3,how;
  DBL dnum;
  DBL ***d3=*d3aray;
...
void TformINT1(FILE *fptr, INT iinit, INT m1, INT **i1aray) 
{ 
  INT i1,n1;
  INT inum;
  INT *i1a=*i1aray;
...
void TformINT2(FILE *fptr, INT iinit, INT m1, INT m2, INT ***i2aray) 
{ 
  INT i1,i2,n1,n2,how;
  INT inum;
  INT **i2a=*i2aray;
...
/*Z added for transformed i2elto array
*/
void TformINT2_inv(FILE *fptr, INT iinit, INT m1, INT m2, INT ***i2aray) 
{ 
  INT i1,i2,n1,n2,how;
  INT inum;
  INT **i2a=*i2aray;
...
/*Z not actually called
*/
void TformINT3(FILE *fptr, INT iinit, INT m1, INT m2, INT m3, INT ****i3aray) 
{ 
  INT i1,i2,i3,n1,n2,n3,how;
  INT inum;
  INT ***i3a=*i3aray;
  ...
  *i3aray=i3a;
  ...
  { 
    ...
    { for(i3=0;i3<n3;i3++)
      { for(i1=0;i1<n1;i1++)
        { for(i2=0;i2<n2;i2++) /*Z bug - fixed index */
          { 
            INTr(fptr,&inum);
            i3a[i1][i2][i3]=inum;
    } } } }
    else if(how==321) /*Z bug - added last else (could be just 'else') */
    { for(i3=0;i3<n3;i3++)
      { for(i2=0;i2<n2;i2++)
        { for(i1=0;i1<n1;i1++) /*Z bug - fixed index */
          { 
            INTr(fptr,&inum);
            i3a[i1][i2][i3]=inum;
    } } } } 
...
FILE *TwriteCHR2(CHR *namep, FILE *fpt, INT ndigit, INT nperli, INT  n1, CHR **c2aray, CHR *name)
{ 
  INT i1;
  INT counter;
  FILE *fptr=fpt;   
...
FILE *TwriteDBL1(CHR *namep, FILE *fpt, INT ndigit, INT nperli, INT  n1, DBL  *d1aray, CHR *name)
{ 
  INT i1;
  INT counter;
  DBL dnum;
...
FILE *TwriteDBL2(CHR *namep, FILE *fpt, INT ndigit, INT nperli, INT ihow, INT n1, INT n2, DBL **d2aray, CHR *name)
{ 
  INT i1,i2;
  INT counter;
  DBL dnum;   
...
FILE *TwriteDBL3(CHR *namep, FILE *fpt, INT ndigit, INT nperli, INT ihow, INT n1, INT n2, INT n3, DBL ***d3aray, CHR *name)
{ 
  INT i1,i2,i3;
  INT counter;
  DBL dnum;   
...
FILE *TwriteINT1(CHR *namep, FILE *fpt, INT ndigit, INT nperli, INT n1, INT *i1aray, CHR *name)
{ 
  INT i1;
  INT counter,istring;
  INT inum;   
...
FILE *TwriteINT2(CHR *namep, FILE *fpt, INT ndigit, INT nperli, INT ihow, INT n1, INT n2, INT **i2aray, CHR *name)
{ 
  INT i1,i2;
  INT counter,istring;
  INT inum; 
...
FILE *TwriteINT3(CHR *namep, FILE *fpt, INT ndigit, INT nperli, INT ihow, INT n1, INT n2, INT n3, INT ***i3aray, CHR *name)
{ 
  INT i1,i2,i3;
  INT counter,istring;
  INT inum; 
...
void TsortDBL1(INT n, DBL *d1x,  INT *i1x)   
{ 
...
void TsortINT(INT n, INT nsort, INT nrear, INT **i2)      
{ 
...
/*Z made extern, call once only from Yinit() */
void initcode(void)
{ 
  INT i,j;
...
void codeINTtoDBL(DBL *d1num, INT *i1num)
{ 
...
void codeDBLtoINT(DBL *d1num, INT *i1num)
{ 
  INT inum,nnum,ndigit;
  DBL dmax,dval;
