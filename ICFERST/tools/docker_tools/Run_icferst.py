...
if testname[-2:] == "-h":
    ...
    print(
        "By default docker runs as the user icferst. Make sure it has the rights to write and read in the working folder."
    )
    exit()
...
pwdpath = os.getcwd()
run_command = (
    "docker run -v " + os.getcwd() + ":/rundir"
)  # run and tell docker which folder it can see
...
run_command += " icferst"  # name of the executable (to be changed to icferst)
run_command += " /rundir/" + testname
run_command = run_command.replace(
    "./", ""
)  # Remove any potential "./" to access a file
print(run_command)
