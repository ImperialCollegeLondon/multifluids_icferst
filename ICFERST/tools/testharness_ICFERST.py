...
sys.path.insert(
    0,
    os.path.abspath(
        os.path.join(os.getcwd(), os.path.dirname(sys.argv[0]), "../..", "python")
    ),
)
print(sys.path)
import fluidity.regressiontest as regressiontest
...
class TestHarness:
    ...
    def decide_fluidity_command(self):
        ...
        for binaryBase in ["multiphase_prototype"]:
            binary = binaryBase
    ...
    def modify_command_line(self, nprocs):
        ...
        def f(s):
            if not flucmd in [None, "multiphase_prototype"]:
                s = s.replace("mutliphase_prototype ", flucmd + " ")
...
if __name__ == "__main__":
    ...
    os.environ["PATH"] = (
        os.path.abspath(os.path.join(os.path.dirname(sys.argv[0]), "..", "bin"))
        + ":"
        + os.environ["PATH"]
    )
    try:
    ...
    if options.justlist:
        ...
        if len(which) > 0:
            print("which {}: {}".format("fluidity", which), end=" ")
