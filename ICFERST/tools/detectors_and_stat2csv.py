...
"""
stat to csv convertor for ICFERST output stat files.
"""
...
def Help():
    ...
    """
    print(
        "Usage: stat2csv [OPTIONS] ... STAT\n"
        + "\n"
        + "Converts a ICFERST .stat file to a .csv file."
        + "\n"
        + "It needs icferst_tools.py in the same folder or in the PTYHON_PATH"
        + "\n"
        + "Options:\n"
        + "\n"
...
def FatalError(message):
    """
    Send an error message to standard error and terminate with a non-zero return
    value.
    """
...
s = icferst_tools.stat_parser(inputFile)
    ...
    for key in input:
        if isinstance(input[key], dict):
            ...
            if len(datum.shape) == 1:
                ...
                raise Exception("Unexpected data rank: " + str(len(datum.shape)))
