...
temp_bak = tracer%val(1,:,:)!<= backup of the tracer field, just in case the petsc bug hits us here, we can retry
  ...
  SUBROUTINE Concentration_assem_solve( state, packed_state, &
       ...
       icomp, saturation, Permeability_tensor_field, nonlinear_iteration, Courant_number )
           ...
           REAL, DIMENSION( : ), allocatable :: CV_RHS_SUB
           type( tensor_field ), pointer :: P, Q
           INTEGER :: IPHASE, its_taken, ipres, cv_nodi
           LOGICAL :: RETRIEVE_SOLID_CTY
           type( tensor_field ), pointer :: den_all2, denold_all2, a, aold, deriv, Component_Absorption
           type( vector_field ), pointer  :: MeanPoreCV, python_vfield
 ...
 ! call MatView(Mmat%petsc_ACV%M,   PETSC_VIEWER_STDOUT_SELF, ipres)
    ...
    !> @author Chris Pain, Pablo Salinas, Asiri Obeysekara, Haiyang Hu
...
! got_free_surf - INDICATED IF WE HAVE A FREE SURFACE - TAKEN FROM DIAMOND EVENTUALLY...
        ...
        type(petsc_csr_matrix)::  CMC_petsc, CMC_petsc2
        ...
        real, DIMENSION (:), allocatable :: Compaction_Length
        ...
        if (is_magma .and. (.not. compute_compaction)) solve_mom_iteratively=.false.
        ...
        ! if (nonlinear_its==1) stokes_max_its=stokes_max_its+20
...
! call MatView(CMC_petsc%M,   PETSC_VIEWER_STDOUT_SELF, ipres)
    ...
    ! if (is_magma) call cal_contribution()
        ...
        if (.not. solve_stokes .or. .not. solve_mom_iteratively ) then
...
!       
  ...
  subroutine cal_contribution()
    ...
    ! call get_option('/material_phase::phase1/scalar_field::BulkComposition/prognostic/initial_condition::WholeMesh/constant', init_bulk)
        ...
        !---------------------------------------------------------------------------
        !> @author Haiyang Hu
        !> @Compute the bulk density which needs to be updated in each non-linear iteration
        !---------------------------------------------------------------------------
        ! subroutine update_bulk_density (state, packed_state, Mdims, bulk_density)
        !     implicit none
        !     type(state_type ), dimension( : ), intent( inout ) :: state
        !     type(state_type ), dimension( : ), intent( inout ) :: packed_state
        !     type(multi_dimensions), intent(in) :: Mdims
        !     type (scalar_field), intent (inout) :: bulk_density
        !     !local variable
        !     type( tensor_field ), pointer ::  saturation_field, density_field
        ...
        !     do cv_inod = 1, Mdims%cv_nonods            
        !         do iphase =1, Mdims%nphase
        !             bulk_density%val(cv_inod)= saturation_field%val(1,iphase,cv_inod)*density_field%val(1,iphase,cv_inod)
        !         end do
        !     end do
        ...
        !---------------------------------------------------------------------------
        subroutine calculate_compaction_length ( state, Mdims, ndgln, UDIFFUSION_ALL, UDIFFUSION_VOL_ALL, Compaction_Length, magma_coupling)
            implicit none
            type(state_type ), dimension( : ), intent( inout ) :: state
            type(multi_ndgln), intent(in) :: ndgln
            type(multi_dimensions), intent(in) :: Mdims
            ...
            real, DIMENSION (:), intent( inout ):: Compaction_Length
            type(coupling_term_coef), intent( in ) :: magma_coupling


            !local variable
            type( scalar_field ), pointer ::  saturation
            integer ELE, CV_ILOC, mat_nod,cv_inod,iphase
            ...
            DO ELE = 1, Mdims%totele
                DO CV_ILOC = 1, Mdims%cv_nloc
                  ...
                  do iphase =1, 1
                    Compaction_Length (mat_nod) = (UDIFFUSION_ALL(1, 1, iphase, mat_nod)/ UDIFFUSION_ALL(1, 1, 2, mat_nod)* &
                    magma_coupling%grain_size**2/magma_coupling%a*max(saturation%val(cv_inod),1e-8)**(magma_coupling%b))**0.5
                  end do
                end DO
              end DO
        ...
        subroutine force_zero_boundary_value(Mdims, field)
            implicit none
            type(multi_dimensions), intent(in) :: Mdims
            type(tensor_field), intent(in) ::  field

            !Local variables
            integer:: ele, u_iloc, xu_inod, u_inod
            !type(mesh_type), pointer :: u_position
            type( vector_field ), pointer :: u_position

            !u_position=> extract_mesh(state, "VelocityMesh")
            u_position=>extract_vector_field(packed_state,"VelocityCoordinate")
            do ele = 1, Mdims%totele
                do u_iloc = 1, Mdims%u_nloc
        ...
        !   if (solver_tolerance<0) then
            ...
            solver_tolerance=solver_tolerance*10**(-no_nonlinear_iteration*solver_tolerance_relax_scale/(max(real(max_nonlinear_iteraion-1),1.0))+solver_tolerance_relax_scale*real(max_nonlinear_iteraion)/(max(real(max_nonlinear_iteraion-1),1.0)))
...
print *, k,':', conv_test
            ...
            field_residuals(:, i) = rhs_p%val(1,:)!P_all%val(1,1,:) - stored_field(:, i) !deltap%val(1,:)
            !##########################Now solve the equations##########################
            i = i + 1
        ...
        subroutine generate_Pivit_matrix_Stokes(state, ndgln,  Mdims, final_phase,  Mmat, MASS_ELE, diagonal_A, upwnd, Pivit_type, Compaction_length, CMC_scale)
          ...
          type (porous_adv_coefs), intent(inout) :: upwnd
          integer, intent(in) :: Pivit_type
          !Local variables
          integer :: j, cv_iloc, cv_loc, imat, u_iloc, u_inod, i
            ...
            !    mu_tmp = ele_val( viscosity, ele )
                ...
                auxR2= 0.
                    do cv_iloc = 1, Mdims%cv_nloc
                        ...
                        auxR2= auxR2+ Compaction_length(imat)**2/dble(Mdims%cv_nloc)
                    end do
            !    auxR = (MASS_ELE(ele)/dble(Mdims%u_nloc)) * auxR
        ...
        subroutine solve_and_update_velocity(Mmat,Velocity, CDP_tensor, U_RHS, diagonal_A)
          ...
          type( tensor_field ), pointer  :: saturation_field
          integer :: ele, u_iloc, u_inod, cv_loc, iphase, idim, cv_iloc
          type(tensor_field):: CDP_tensor_copy
          ...
          if (compute_compaction) then
            ...
            print *,'two_phase velocity solver'
            packed_vel = as_packed_vector(Velocity)            
            if (is_magma) then ! for magma original formulation, dP is scaled with saturation
                call allocate(CDP_tensor_copy,velocity%mesh,"CDP_copy",dim = (/velocity%dim(1), final_phase/))
                CDP_tensor_copy%val=CDP_tensor%Val
                ...
                do ele = 1, Mdims%totele
                    ...
                    u_inod = ndgln%u((ele-1)*Mdims%u_nloc+u_iloc)
                        ...
                        cv_loc = ndgln%cv((ele-1)*Mdims%cv_nloc+cv_iloc)
                            do iphase = 1, final_phase
                                DO idim = 1, Mdims%ndim
                                    ! the second phase of upwnd%inv_adv_coef containts phi/c
                                    CDP_tensor%val(idim, iphase, u_inod) = CDP_tensor_copy%val(idim, iphase, u_inod)*saturation_field%val(1, iphase, cv_loc)
                                END DO
                            end do
                        end do
                    end do
...
! call MatView(Mmat%DGM_PETSC%M,   PETSC_VIEWER_STDOUT_SELF, ipres)
    ...
    call deallocate(packed_vel); call deallocate(rhs)
        ...
        !---------------------------------------------------------------------------
        subroutine project_velocity_to_affine_space(Mdims, Mmat, Mspars, ndgln, velocity, Pressure, deltap, cdp_tensor, rhs_p, upwnd)
          ...
          type( scalar_field ), pointer :: saturation
          type(tensor_field) :: cdp_tensor2 ! the correction term to the original pressure tensor
          integer :: u_iloc, u_inod, idim, iphase, cv_iloc, ele, imat, cv_loc
          real :: auxR
          if (compute_compaction) then  ! for compaction formulation, we correct the pressure gradient instead of the solid velocity
            ! recalculate the residue from the newest pressure and velocity
            rhs_p%val=0.
            call compute_DIV_U(Mdims, Mmat, Mspars, velocity%val, INV_B, rhs_p)
            if (compute_compaction) call include_Laplacian_P_into_RHS(Mmat, Pressure, rhs_p, deltap)
            rhs_p%val = - rhs_p%val

            ! calculate the current pressure gradient
            CDP_tensor%val=0.
            call C_MULT2_MULTI_PRES(Mdims, final_phase, Mspars, Mmat, P_ALL%val, CDP_tensor)

            ! Calculate the corection term for the pressure gradient
            ! cmc_petsc2 is CT phi^2/c C, rhs_p is the latest residue from the continuity equation
            call solve_and_update_pressure(Mdims, rhs_p, P_all%val, deltap, cmc_petsc2, diagonal_CMC%val, update_pres= .false.)
            call allocate(cdp_tensor2,velocity%mesh,"CDP",dim = (/velocity%dim(1), final_phase/)); call zero(cdp_tensor2)
            call C_MULT2_MULTI_PRES(Mdims, final_phase, Mspars, Mmat, deltap%val, CDP_tensor2)
            ! multiply CDP_tensor2 with phi^2/c
            ! saturation => extract_scalar_field(state(2), "PhaseVolumeFraction")
            ! do ele = 1, Mdims%totele
            !     do u_iloc = 1, Mdims%u_nloc
            !         u_inod = ndgln%u((ele-1)*Mdims%u_nloc+u_iloc)
            ...
            !                 DO idim = 1, Mdims%ndim
            !                     ! the second phase of upwnd%inv_adv_coef containts phi/c
            !                     CDP_tensor2%val(idim, iphase, u_inod) = CDP_tensor2%val(idim, iphase, u_inod)*upwnd%inv_adv_coef(1,1,2,imat)*saturation%val(cv_loc)
            !                 END DO
            !             end do
            !         end do
            !     end do
            ...
            ! Generate pivit matrix which only contains mass matrix
            call generate_Pivit_matrix_Stokes(state, ndgln, Mdims, final_phase, Mmat, MASS_ELE, diagonal_A, upwnd, 1)
            DU_VEL = 0.
            CALL Mass_matrix_MATVEC( DU_VEL, Mmat%PIVIT_MAT, CDP_tensor2%val, Mdims%ndim, final_phase , Mdims%totele, Mdims%u_nloc, ndgln%u )
            CDP_tensor%val=CDP_tensor%val+DU_VEL
        else
            !Perform C * DP
            ...
            DU_VEL = 0.
            CALL Mass_matrix_MATVEC( DU_VEL, Mmat%PIVIT_MAT, CDP_tensor%val, Mdims%ndim, final_phase , Mdims%totele, Mdims%u_nloc, ndgln%u )
            !Update the velocity
        ...
        !---------------------------------------------------------------------------
        !> @author Pablo Salinas, Haiyang Hu
        !> @brief Calculates the divergence of the velocity* density (added for three phase) by multipliying it by the Ct matrix
        !---------------------------------------------------------------------------
      subroutine compute_DIV_U(Mdims, Mmat, Mspars, velocity, INV_B, rhs_p, force_transpose_C, den, drhog_tensor)
        implicit none
        type(multi_dimensions), intent(in) :: Mdims
        type (multi_sparsities), intent(in) :: Mspars
        ...
        type (scalar_field), optional, intent(in) :: den
        REAL, optional, DIMENSION( :, :, : ), intent(in) :: drhog_tensor

        !Local variables
        REAL, DIMENSION( :, :, : ), allocatable :: scaled_velocity
        integer :: ele, cv_iloc, cv_loc, u_iloc, u_inod
        real :: auxR
...
!###############################################################################################################
      ...
      subroutine include_Laplacian_P_into_RHS(Mmat, Pressure, rhs_p, vfield)
        ...
        call mult(vfield, CMC_petsc2, vpressure) !Mmat%petsc_ACV
...
! NEED TO CHANGE RETRIEVE_SOLID_CTY TO MAKE AN OPTION
        ...
        call CV_ASSEMB( state, packed_state, &
            one_or_n_in_press, Mdims, CV_GIdims, CV_funs, Mspars, ndgln, Mdisopt, Mmat, upwnd, &
            tracer, velocity, density, multi_absorp, &
            DIAG_SCALE_PRES, DIAG_SCALE_PRES_COUP, INV_B, &
       ...
       IF( GOT_DIFFUS .or. get_gradU ) THEN
        ...
        Loop_Elements: DO ELE = 1, Mdims%totele ! VOLUME integral
            ...
            DO MAT_ILOC = 1, Mdims%mat_nloc
                ...
                ENDIF
                IF ( GOT_DIFFUS ) THEN
                    LOC_UDIFFUSION( :, :, :, MAT_ILOC ) = UDIFFUSION_ALL( :, :, 1:final_phase, MAT_INOD )
