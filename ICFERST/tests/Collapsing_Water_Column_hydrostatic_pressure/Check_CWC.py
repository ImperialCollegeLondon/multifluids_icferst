...
def getAnalytical_interpolated(Analytical_X, Analytical_Y, position):
    ...
    a = (Analytical_Y[k - 1] - Analytical_Y[k]) / (
        Analytical_X[k - 1] - Analytical_X[k]
    )
    getAnalytical_interpolated = (
        a * (position - Analytical_X[k - 1]) + Analytical_Y[k - 1]
    )
...
Analytical_X = []
Analytical_Y = []
Analytical = open("Semi-Analytical")
