...
os.system(binpath + " " + path + "/co2_sat_mass_conservation_adapt_rhoT.mpml")
...
RHO0 = {"water": 1000.0, "CO2": 385.0}


def rho_field(pts, phase):
    """The EXACT prescribed-driver density: rho0*(1 + 0.15*(Z_TOP - z)/Z_TOP).
    Compressible phases conserve int(phi*rho*S)dV, so the check must integrate
    in that measure; the driver is analytic, so no vtu density field is needed."""
    return RHO0[phase] * (1.0 + 0.15 * (120.0 - pts[:, 2]) / 120.0)
...
def measure(fname):
    ...
    for p in Phases:
        ...
        out[p] = math.fsum(
            np.asarray(vol * por * rS[tconn].mean(axis=1), dtype=np.float64)
        )
...
if Passed:
    for i in range(AutoNumber + 1):
        ...
        print(
            "  dump %3d: nodes = %7d, " % (i, n)
            + ", ".join("phi*V*S[{}] = {:.10e}".format(p, out[p]) for p in Phases)
            + ", max|1-sum| = %.2e" % sres
        )
...
if Passed:
    for p in Phases:
        ...
        worst = max(
            (
                abs(masses[p][i] - masses[p][i - 1]) / M0
                for i in range(1, len(masses[p]))
            ),
            default=0.0,
        )
        total = abs(masses[p][-1] - M0) / M0
        print(
            "%s: worst per-event drift %.3e (tol %.1e), total %.3e (tol %.1e)"
    ...
    print(
        "max |1 - sum_p S_p|          : %.3e (tolerance %.1e)"
        % (worst_sum, Tolerance_sum_one)
    )
    print("saturation range             : [{:.3e}, {:.3e}]".format(smin_all, smax_all))
    print(
        "max far-field CO2 S (>%.0f m) : %.3e (tolerance %.1e)"
...
if Passed:
    print(
        "Compressible (rho-field) saturation mass conservation across adaptivity works OK"
    )
else:
    print(
        "Compressible (rho-field) saturation mass conservation across adaptivity does NOT work"
    )
