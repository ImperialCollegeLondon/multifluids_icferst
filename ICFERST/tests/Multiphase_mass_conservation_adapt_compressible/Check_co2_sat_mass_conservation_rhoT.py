...
def measure(fname):
    ...
    for p in Phases:
        ...
        out[p] = math.fsum(np.asarray(vol * por * rS[tconn].mean(axis=1),
                                      dtype=np.float64))
...
if Passed:
    print('Compressible (rho-field) saturation mass conservation across adaptivity works OK')
else:
    print('Compressible (rho-field) saturation mass conservation across adaptivity does NOT work')
