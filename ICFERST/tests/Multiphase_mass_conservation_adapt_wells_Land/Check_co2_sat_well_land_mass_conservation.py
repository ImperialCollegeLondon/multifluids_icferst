...
if imm_hist:
    if any(v is None for v in imm_hist):
        print(
            "CV_ImmobileFraction not found: add the diagnostic to the CO2 "
        ...
        if imm0 < Imm_floor:
            print(
                "Initial CO2 immobile fraction %.3e below floor %.1e: the "
            ...
            worst_event = max(
                (
                    (imm_hist[i - 1] - imm_hist[i]) / imm_hist[i - 1]
                    for i in range(1, len(imm_hist))
                    if imm_hist[i - 1] > Imm_floor
                ),
                default=0.0,
            )
