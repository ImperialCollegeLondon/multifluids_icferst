...
def analytical_solution(X_coords):
    ...
    term1 = erfc((coords - (gamma * t)) / (2.0 * sqrt(t)))
    term2 = (np.exp(gamma * coords)) * erfc((coords + (gamma * t)) / (2.0 * sqrt(t)))
    term3 = 1.0 + (0.5 * gamma * (2.0 - coords + (gamma * t)))
    term4 = erfc((2.0 - coords + (gamma * t)) / (2.0 * sqrt(t)))
    term5 = (
        gamma
        * (sqrt(t / pi))
        * np.exp(-((2.0 - coords + (gamma * t)) ** 2.0) / (4.0 * t))
    )
...
Experimental_X = np.linspace(
    0, 1.0, num=nodes, endpoint=True, retstep=False, dtype=None
)
