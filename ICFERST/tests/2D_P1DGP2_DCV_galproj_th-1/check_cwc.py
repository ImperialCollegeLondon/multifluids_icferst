    ...
    for f in filelist:
        ...
        for x in range(len(xarray)):
            coordinates = numpy.concatenate(
                (numpy.ones((len(y), 1)) * xarray[x], y, numpy.zeros((len(y), 1))), 1
            )
            waterdepths.append(
                sum(
                    vtu.ProbeData(
                        coordinates, "Component1::ComponentMassFractionPhase1"
                    )
                )[0]
                * delta
            )
...
for x in range(len(xarray)):
    ...
    print(
        ...
        TOLERANCE_H,
        ", ERROR=",
        abs(numpy.std(numpy.array(experiment[:, 2]) - numpy.array(results[:, 2 + x]))),
    )
    H_check = (
        abs(numpy.std(numpy.array(experiment[:, 2]) - numpy.array(results[:, 2 + x])))
        < TOLERANCE_H
...
for p in range(len(parray)):
    ...
    for i in ts:
        ...
        i2 = next(
            j
            for j, _ in enumerate(experiment.item(0)["ElapsedTime"]["value"])
            if numpy.isclose(_, i, tol)
        )
    ...
    if plotting == True:
        ...
        pylab.axis([0.0, 1.0, -1000.0, 10000.0])
        pylab.legend(("Experiment", "Model"), loc="upper left")
    ...
    print(
        ...
        TOLERANCE_P,
        ", ERROR=",
        abs(numpy.std(numpy.array(data_o2) - numpy.array(data_2))),
    )
