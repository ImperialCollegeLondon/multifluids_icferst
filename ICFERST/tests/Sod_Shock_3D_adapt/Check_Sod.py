...
#### by ASIRI OBEYSEKARA (2019/2020)
#### BENCHMARKS 3D adaptive shock modelling capability using the Sod Shock problem
...
os.system("make")
os.system(binpath + " " + path + "/sod_3da.mpml")
...
for j in range(points.GetNumberOfPoints()):
    ...
    if ID[0] > 0:
        ...
        FSV.append(data.GetPointData().GetScalars(data_name_p).GetTuple(j))
...
for i in range(len(Experimental_X)):
    if i == 0:  # The first position is exact, so no need to interpolate
        ...
        continue
    Experimental_X[i] = Experimental_X[
        i
    ]  # + 0.5#In this test case the origin is in -0.5
...
print(L1_norm)
print(L2_norm)
if L1_norm > Tolerance_L1_NORM:
    Passed = False
if L2_norm > Tolerance_L2_NORM:
    Passed = False
