...
contains
  ...
  subroutine set_boundary_condition_values(psi, positions)
    ...
    call allocate(bc_field, bc_surface_mesh, name='value')

    ! set it to some constant
    call set(bc_field, BC_CONST_VALUE)
    ...
    call set_from_python_function(bc_field, BC_PYTHON_FUNCTION, bc_positions, 0.0)
  ...
  subroutine assemble_boundary_contribution(rhs, positions, psi, bc_name)
    ...
    type(scalar_field), pointer:: bc_surface_field
    ...
    real, dimension(face_loc(psi,1)) :: bc_val_face
    ...
    do ele=1, ele_count(bc_surface_field)
      ...
      call addto(rhs, psi_face_nodes, matmul(face_mat, bc_val_face))
    end do

  end subroutine assemble_boundary_contribution

  subroutine read_command_line(filename, degree, quad_degree)
    ! Read the input filename, degree and quadrature degree on the command
    ! line.
    character(len=*), intent(out) :: filename
    integer, intent(out) :: degree, quad_degree
    character(len=256) :: degree_buffer
    integer :: status
