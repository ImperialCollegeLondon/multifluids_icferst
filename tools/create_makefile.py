...
def generate_dependencies(fortran):
    ...
    while len(fortran) < setsize and len(fortran) > 0:
        print("Generating dependencies, " + str(len(fortran)) + " to go.")
        setsize = len(fortran)
        ...
        for f in fortran:
            ...
            if pipe.close() == None:
                ...
                for dep in dep_exclusions:
                    this_deps.remove_dep_by_rule(lambda x: x.startswith(dep))
...
if __name__ == "__main__":
    ...
    try:
        ...
        fortran = set(glob.glob("*.[fF]90")).difference(set(options.exclude.split()))
