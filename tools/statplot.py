...
from matplotlib.backends.backend_gtk3 import NavigationToolbar2GTK3 as NavigationToolbar  # noqa: E402
from matplotlib.backends.backend_gtk3agg import FigureCanvasGTK3Agg as FigureCanvas  # noqa: E402
...
class StatplotWindow(Gtk.Window):
    ...
    def FormatAxis(self, ax2fmt, scale):
        ...
        if scale == "linear":
            self.ax.ticklabel_format(
                style="sci", axis=ax2fmt, scilimits=(0, 0), useMathText=True
            )
            ...
            symLogLoc = tck.SymmetricalLogLocator(
                subs=numpy.arange(2, 10), linthresh=axMin * 10 ** (axRange / 2), base=10
            )
            ...
            curAxis.set_minor_formatter(logFmt)
        self.ax.set_xlabel(
            self.xCombo.get_child().get_text(), fontweight="bold", fontsize=20
        )
        self.ax.set_ylabel(
            self.yCombo.get_child().get_text(), fontweight="bold", fontsize=20
        )
        ...
        self.ax.xaxis.get_offset_text().set(
            fontsize=13, fontweight="bold", color="xkcd:black"
        )
        self.ax.yaxis.get_offset_text().set(
            fontsize=13, fontweight="bold", color="xkcd:black"
        )
    ...
    def KeyPressed(self, widget, event):
        key = event.string
        if self.xCombo.get_child().has_focus() or self.yCombo.get_child().has_focus():
            pass
        elif key == "r":
            self.RefreshData()
            self.PlotData(
                "update", self.PlotType, self.ax.get_xscale(), self.ax.get_yscale()
            )
        elif key == "q":
            self.destroy()
        elif key == "x" or key == "y":
    ...
    def PlotData(self, action, type, xscale, yscale):
        ...
        if numpy.unique(self.xData).size == 1 and numpy.unique(self.yData).size == 1:
            type = "marker"
        if action == "create":
            (self.statplot,) = self.ax.plot(
                self.xData, self.yData, linewidth=2, color="xkcd:light purple"
            )
        elif action == "update":
    ...
    def ReadData(self):
                ...
                for num, line in enumerate(fid):
                    if line.startswith("<field"):
                        ...
                        if info.attrib["statistic"] == "value":
                            ...
                            [x in info.attrib for x in ["components", "material_phase"]]
                        ):
                            for i in range(int(info.attrib["components"])):
                                entries.append(
                                    "{e[material_phase]}%{e[name]}"
                                    "%{e[statistic]}%i".format(e=info.attrib, i=i)
                                )
                            ...
                            entries.append(
                                "{e[name]}%{e[statistic]}".format(e=info.attrib)
                            )
        ...
        if self.statfile[0].endswith("stat"):
            ...
            values = numpy.fromfile(self.statfile[0] + ".dat", dtype=numpy.float64)
            ncols = entries.size
    ...
    def RefreshData(self):
        with self.xCombo.handler_block(self.xCon), self.yCombo.handler_block(self.yCon):
