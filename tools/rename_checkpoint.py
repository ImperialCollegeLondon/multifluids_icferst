...
def main():
    ...
    filelist = (
        glob.glob(base_filename + "_checkpoint_*[0-9].vtu")
        + glob.glob(
            base_filename
            + "_checkpoint_*[0-9]/"
            + base_filename
            + "_checkpoint_*[0-9].vtu"
        )
        + glob.glob(base_filename + "_checkpoint_*[0-9].pvtu")
    )
    ...
    for i in range(len(filelist)):
        if filelist[i][-4:] == ".vtu":
            filesplit = (
                filelist[i]
                .split(".vtu")[0]
                .split(base_filename + "_checkpoint_")[-1]
                .split("_")
            )
            # serial vtus
            if len(filesplit) == 1:
                ...
                if os.path.exists(filelist[i].split(".pvtu")[0]):
                    if os.path.exists(base_filename + "_" + str(newindex)):
                        if verbose:
                            print(
                                "backing up",
                                base_filename + "_" + str(newindex) + "/*.vtu",
                                "to",
                                base_filename + "_" + str(newindex) + "/*.vtu.bak",
                            )
                        for f in glob.glob(
                            base_filename + "_" + str(newindex) + "/*.vtu"
                        ):
                    ...
                    try:
                        ...
                        if verbose:
                            print(
                                filelist[i].split(".pvtu")[0],
                                "not removed, directory may not be empty",
                            )
                    ...
                    if lineindex != -1:
                        ...
                        if len(processorsplit) == 2:
                            newline = (
                                ...
                                + str(newindex)
                                + "_"
                                + str(processorsplit[-1])
                                + ".vtu"
                                + lines[line][lineindex:].split(".vtu")[-1]
                            )
                            lines[line] = newline
