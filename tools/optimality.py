...
def run_model(m, opt_options, model_options):
    ...
    if superspud(
        model_options, "libspud.have_option('/adjoint/controls/load_controls')"
    ):
...
def get_custom_controls(opt_options):
    ...
    for i in range(nb_controls):
        cname = superspud(
            opt_options,
            "libspud.get_option('/control_io/control[" + str(i) + "]/name')",
        )
        ctype = superspud(
            opt_options,
            "libspud.get_option('/control_io/control[" + str(i) + "]/type/name')",
        )
        # With the custom type, the user specifies python function to initialise the controls.
        if ctype == "custom":
            initial_control_code = superspud(
                opt_options,
                "libspud.get_option('/control_io/control["
                + str(i)
                + "]/type::custom/initial_control')",
            )
            d = {}
...
def read_default_controls(opt_options, model_options):
    ...
    for i in range(nb_controls):
        ...
        if ctype == "default":
            ...
            for ctrl_file in glob.iglob(
                "control_" + simulation_name + "_" + cname + "_[0-9]*.pkl"
            ):
                try:
                    timestep = int(
                        ctrl_file.strip()[
                            len("control_" + simulation_name + "_" + cname + "_") : len(
                                ctrl_file
                            )
                            - 4
                        ]
                    ...
                    print(
                        "The control file ",
                        ctrl_file,
                        " does not conform the standard naming conventions for control files.",
                    )
                    exit()
                f = open(ctrl_file, "rb")
...
def read_default_control_bounds(opt_options, model_options):
    ...
    for i in range(nb_controls):
        ...
        for k in m_bounds.keys():
            have_bound[k] = superspud(
                model_options,
                "libspud.have_option('/adjoint/controls/control["
                + str(i)
                + "/bounds/"
                + k
                + "')",
            ...
            for ctrl_file in glob.iglob(
                "control_" + simulation_name + "_" + cname + "_" + k + "_[0-9]*.pkl"
            ):
                try:
                    timestep = int(
                        ctrl_file.strip()[
...
def read_control_derivatives(opt_options, model_options):
    ...
    for i in range(nb_controls):
        ...
        if ctype == "default":
            ...
            if act_flag == False:
                print(
                    "Warning: Found no control derivative file for control ", cname, "."
                )
        elif ctype == "custom":
            control_derivative_code = superspud(
                ...
                "libspud.get_option('/control_io/control["
                + str(i)
                + "]/type::custom/control_derivative')",
            )
            d = {}
                ...
                if k[0] == cname:
                    ...
                    if debug:
                        ...
                        if m[k].shape != m_old.shape:
                            print(
                                "Error: The shape of the control in ",
                                file_name,
                                " changed.",
                            )
                            exit()
...
def check_option_consistency(opt_options, model_options):
    ...
    for i in range(nb_controls):
        ...
        if ctype == "custom":
            if superspud(
                model_options,
                "libspud.have_option('/adjoint/controls/control::" + cname + "')",
            ):
                print(
                    "The custom control "
                    + cname
                    + " is a default control in the model option tree."
                )
                exit()
        elif ctype == "default":
            if not superspud(
                ...
                print(
                    "The default control "
                    + cname
                    + " was not found in the model option tree."
                )
                exit()
        else:
            print("Unknown control type " + ctype + ".")
...
def check_control_consistency(m, djdm, m_bounds=None):
    ...
    if m_keys != djdm_keys:
        ...
        print(
            "Check the consistency of the control definition in the model and the optimality configuration."
        )
        exit()
    ...
    if m_bounds != None:
        ...
        for bound_type in bound_types:
            ...
            if m_keys != m_bounds_keys:
                print(
                    "Error: The controls are not consistent with the control ",
                    bound_type,
                    ".",
                )
                print("The controls are:", m_keys)
...
def have_bounds(opt_options, model_options):
    ...
    for i in range(nb_controls):
        ...
        have_bounds = have_bounds or superspud(
            model_options,
            "libspud.have_option('/adjoint/controls/control[" + cname + "]/bounds')",
        )
    return have_bounds
...
def optimisation_loop(opt_options, model_options):
    ...
    def test_serialise():
        x = {
            ...
            "b": numpy.random.rand(3, 2, 4, 5),
            "c": numpy.random.rand(1),
        }
    ...
    def pure_J(m_serial, m_shape):
        ...
        if not functional_name in s:
            print(
                "The functional '",
                functional_name,
                "' does not exist in the stat file.",
            )
    ...
    def pure_dJdm(m_serial, m_shape):
        if verbose:
            print(
                "Running forward/adjoint model for functional derivative evaluation (<function pure_dJdm>)"
            )
        m = unserialise(m_serial, m_shape)
        run_model(m, opt_options, model_options)
    ...
    def check_gradient(m_serial, m_shape):
        ...
        for i in range(nb_tests):
            ...
            grad_errors.append(
                abs(
                    j_unpert
                    + numpy.dot(djdm_unpert, perturbation_vec * perturbation)
                    - J(m_pert, m_shape)
                )
            )
        ...
        stat_writer[(functional_name + "_gradient_error", "convergence")] = min(
            grad_conv
        )
        stat_writer.write()
    ...
    algo = superspud(
        opt_options,
        "libspud.get_option('optimisation_options/optimisation_algorithm[0]/name')",
    )
    ...
    if superspud(
        ...
        superspud(
            model_options,
            [
                "libspud.delete_option('/adjoint/controls/load_controls')",
                "libspud.write_options('" + model_file + "')",
            ],
        )
    ...
    if superspud(
        opt_options,
        "libspud.have_option('/adjoint/functional::"
        + functional_name
        + "/disable_adjoint_run')",
    ):
        superspud(
            opt_options,
            "libspud.delete_option('/adjoint/functional::"
            + functional_name
            + "/disable_adjoint_run')",
        )
    ...
    superspud(
        model_options,
        [
            "libspud.add_option('/adjoint/controls/load_controls')",
            "libspud.write_options('" + model_file + "')",
        ],
    )
    ...
    if algo == "BFGS":
        ...
        tol = superspud(
            opt_options,
            "libspud.get_option('/optimisation_options/optimisation_algorithm::BFGS/tolerance')",
        )
        maxiter = None
        if superspud(
            opt_options,
            "libspud.have_option('/optimisation_options/optimisation_algorithm::BFGS/iterations')",
        ):
            maxiter = superspud(
                opt_options,
                "libspud.get_option('/optimisation_options/optimisation_algorithm::BFGS/iterations')",
            )
        res = scipy.optimize.fmin_bfgs(
            ...
            maxiter=maxiter,
            args=(m_shape,),
            callback=lambda m: callback(m, m_shape),
        )
    ...
    ################################
        ...
        tol = superspud(
            opt_options,
            "libspud.get_option('/optimisation_options/optimisation_algorithm::NCG/tolerance')",
        )
        maxiter = None
        if superspud(
            opt_options,
            "libspud.have_option('/optimisation_options/optimisation_algorithm::NCG/iterations')",
        ):
            maxiter = superspud(
                opt_options,
                "libspud.get_option('/optimisation_options/optimisation_algorithm::NCG/iterations')",
            )
    ...
    ################################
        ...
        if superspud(
            opt_options,
            "libspud.have_option('/optimisation_options/optimisation_algorithm::L-BFGS-B/tolerance')",
        ):
            pgtol = superspud(
                opt_options,
                "libspud.get_option('/optimisation_options/optimisation_algorithm::L-BFGS-B/tolerance')",
            )
            opt_args["pgtol"] = pgtol
        if superspud(
            opt_options,
            "libspud.have_option('/optimisation_options/optimisation_algorithm::L-BFGS-B/factr')",
        ):
            factr = superspud(
                opt_options,
                "libspud.get_option('/optimisation_options/optimisation_algorithm::L-BFGS-B/factr')",
            )
            opt_args["factr"] = factr
        if superspud(
            opt_options,
            "libspud.have_option('/optimisation_options/optimisation_algorithm::L-BFGS-B/memory_limit')",
        ):
            memory_limit = superspud(
                opt_options,
                "libspud.get_option('/optimisation_options/optimisation_algorithm::L-BFGS-B/memory_limit')",
            )
            opt_args["m"] = memory_limit
        if superspud(
            opt_options,
            "libspud.have_option('/optimisation_options/optimisation_algorithm::L-BFGS-B/maximal_functional_evaluations')",
        ):
            maxfun = superspud(
                opt_options,
                "libspud.get_option('/optimisation_options/optimisation_algorithm::L-BFGS-B/maximal_functional_evaluations')",
            )
            opt_args["maxfun"] = maxfun
        if superspud(
            opt_options,
            "libspud.have_option('/optimisation_options/optimisation_algorithm::L-BFGS-B/verbosity')",
        ):
            iprint = superspud(
                opt_options,
                "libspud.get_option('/optimisation_options/optimisation_algorithm::L-BFGS-B/verbosity')",
            )
...
def main():
    ...
    superspud(
        opt_file,
        [
            "libspud.set_option('/model/option_file', '" + model_file + "')",
            "libspud.write_options('" + opt_file + "')",
        ],
    )
