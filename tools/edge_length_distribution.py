...
def usage():
    ...
    parser.add_option(
        "-c",
        "--cumulative",
        dest="plot_cumulative",
        action="store_true",
        help="plots a histogram of the cumulative total of edge lengths for all vtus",
    )
    #
    parser.add_option(
        "-p",
    ...
    parser.add_option(
        ...
        dest="use_pvtu",
        action="store_true",
        help="uses pvtus instead of vtus",
    )
    return parser
...
def pair_off(nodes):
    # for a node list writes out all the pairs
