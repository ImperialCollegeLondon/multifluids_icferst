...
class TriangleReader(FileDataSource):
    ...
    output_info = PipelineInfo(
        datasets=["unstructured_grid"], attribute_types=["any"], attributes=["scalars"]
    )

    # The active point scalar name.
    point_scalars_name = DEnum(
        values_name="_point_scalars_list", desc="scalar point data attribute to use"
    )

    # The active cell scalar name.
    cell_scalars_name = DEnum(
        values_name="_cell_scalars_list", desc="scalar cell data attribute to use"
    )
    ...
    view = View(Item(name="point_scalars_name"), Item(name="cell_scalars_name"))
    ...
    def _read_node_file(self):
        ...
        if boundary_marker:
            boundary_marker_array = data_array[
                :, (dimensions + attributes + 1) : (dimensions + attributes + 2)
            ]
    ...
    def _read_face_edge_file(self, extension):
        ...
        """
        file_name = "{}{}".format(self._basename, extension)
    ...
    def _read_ele_file(self):
        ...
        for i in range(attributes):
            attribute_array = data_array[
                :, (i + nodes_per_tet_tri + 1) : (i + nodes_per_tet_tri + 2)
            ]
