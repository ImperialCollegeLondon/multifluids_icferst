...
from numpy import fromfile, ones, shape, zeros
...
def readstat_3d():
    ...
    for i in range(num_detectors):
        n = i + 1
        last_locations_error[0, i] = s[array_name + str(n).zfill(zfill_length)][
            "position"
        ][0][-1]
        last_locations_error[1, i] = s[array_name + str(n).zfill(zfill_length)][
            "position"
        ][1][-1]
        last_locations_error[2, i] = s[array_name + str(n).zfill(zfill_length)][
            "position"
        ][2][-1]
